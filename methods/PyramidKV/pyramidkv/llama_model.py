...
def llama_attn_forward_PyramidKV(
    ...
    **kwargs,
    ...
    # re-init timing on every prefill or first decode
    ...
    if past_key_value is not None:
        ...
        if key_states.shape[-2] == kv_seq_len:
            ...
            key_states_compress, value_states_compress = self.kv_cluster.update_kv(key_states, query_states, value_states, attention_mask, self.num_key_value_groups, tm=tm)
            if tm is not None:
            ...
            # Save key/query on kv_cluster for recall tracking (decode path bypasses update_kv)
            self.kv_cluster._recall_key = key_states
            self.kv_cluster._recall_query = query_states
...
def llama_flash_attn2_forward_PyramidKV(
    ...
    **kwargs,
    ...
    tm = getattr(getattr(self, "config", None), "_time_manager", None)
    init_pyramidkv(self, num_hidden_layers=self.config.num_hidden_layers)
    ...
    if past_key_value is not None:
        cache_kwargs = {"sin": sin, "cos": cos, "cache_position": kwargs.get("cache_position")}  # Specific to RoPE models
        # key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
    ...
    if tm is not None:
        if is_prefill:
            tm.record_pref_pure_start(self.layer_idx)
        else:
    ...
    if tm is not None:
        if is_prefill:
            tm.record_pref_pure_end(self.layer_idx)
        else:
