...
from PyramidKV.experiments.RULER.eval.utils import load_data, dump_jsonl
...
def get_pred_and_ref(
    ...
    for line in tqdm(lines):
        ...
        metadata = line.get(metadata_field)
        if isinstance(metadata, dict):
            index = metadata.get('id', line['index'])
        else:
            index = line['index']
...
def write_evaluation(results: dict):
    ...
    # 计算分数平均值（只统计有分数的任务，忽略空分数）
    score_floats = [float(s) for s in score if s not in (None, '', '-')]
    avg_score = sum(score_floats) / len(score_floats) if score_floats else 0.0
    dfs = [
        ['Tasks'] + tasks,
        ['Score'] + score,
        ['Nulls'] + nulls,
        ['Average'] + ["{:.2f}".format(avg_score)] + ["" for _ in range(len(tasks)-1)]
    ]
