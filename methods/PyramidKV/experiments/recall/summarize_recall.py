...
for path in sorted(glob.glob(os.path.join(root, "**", "*_attn_ratios.csv"), recursive=True)):
    ...
    comps = rel.split("/")
    # Structure variants:
    # 4 parts: <method>/<model>/budget<budget>/<dataset>_attn_ratios.csv  (HeadKV)
    # 3 parts: <model>/budget<budget>/<dataset>_attn_ratios.csv           (CakeKV)
    if len(comps) == 4:
        ...
        continue
    budget = budget_dir.replace("budget", "")
    dataset = os.path.basename(path).replace("_attn_ratios.csv", "")
