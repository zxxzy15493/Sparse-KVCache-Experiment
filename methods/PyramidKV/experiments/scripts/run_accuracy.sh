#!/bin/bash
# =============================================================================
# Unified accuracy benchmark runner for PyramidKV
...
#   bash run_accuracy.sh --dataset ruler --model llama --cache_sizes "4096"
# =============================================================================
set -euo pipefail
...
DATASET=""
MODEL=""
CACHE_SIZES=()         # space-separated list (maps to max_capacity_prompts)
MAX_SAMPLES=500        # max samples (longbench / ruler)
WINDOW_SIZE=32
DEVICE=0

# --- PyramidKV parameters ---
PYRAM_BETA=20
KERNEL_SIZE=5
POOLING="maxpool"
FLOOR=0.2
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --help|-h|help)
            ...
            echo ""
            echo "  Optional (PyramidKV):"
            echo "    --pyram_beta      PyramidKV pyram_beta         (default: 10)"
            echo "    --kernel_size     kernel size for pooling      (default: 5)"
            echo "    --pooling         pooling type (maxpool/avgpool)(default: maxpool)"
            echo "    --floor           floor value                  (default: 0.2)"
            echo "    --skip            skip layers                  (default: 0)"
            echo "    --normalize       normalize attention scores   (default: false)"
            echo ""
            echo "  Optional (RULER):"
...
if [[ -z "$SUBCOMMAND" && ! " ${VALID_DATASETS[*]} " =~ " ${DATASET} " ]]; then
...
# RULER: llama, qwen(1m) (short names from config_models.sh)
...
# LongBench-v2: qwen(1m) (keys from config/model2path.json)
...
run_gsm8k() {
    ...
    local log_dir="${EXPERIMENTS_DIR}/GSM8K/log/PyramidKV/${model_tag}/budget${cache_size}"
    local save_dir="${EXPERIMENTS_DIR}/GSM8K/results/PyramidKV/${model_tag}/budget${cache_size}"
...
run_longbench() {
    ...
    local save_dir="./results/results_pyramidkv_cap${cache_size}_beta${PYRAM_BETA}"
...
run_longbench_v2() {
    ...
    local save_dir="results/pyramidkv_cap${cache_size}_beta${PYRAM_BETA}"
...
" 2>/dev/null || echo "$lbv2_model")
    ...
    if [[ -f "$result_file" ]]; then
        mkdir -p results
        cp "$result_file" "results/cap${cache_size}_${model_key}.jsonl"
        python result.py
    else
...
run_ruler() {
    ...
    for MAX_SEQ_LENGTH in "${RULER_SEQ_LENGTHS[@]}"; do
        ...
        local pred_dir="${results_dir}/pred/base${cache_size}"
...
if [[ -n "$SUBCOMMAND" ]]; then
    ...
    echo "============================================================"
    echo "PyramidKV Accuracy Runner"
    echo "  Dataset:      $DATASET"
    echo "  Model:        $MODEL"
    echo "  Cache sizes:  ${CACHE_SIZES[*]}"
    echo "  Max samples:  $MAX_SAMPLES"
    echo "  Window size:  $WINDOW_SIZE"
    echo "  Pyram beta:   $PYRAM_BETA"
