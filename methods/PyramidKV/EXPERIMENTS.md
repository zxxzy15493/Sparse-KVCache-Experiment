...
PyramidKV's **accuracy evaluation** is integrated into the project's unified evaluation framework via `run_accuracy.sh`, which provides preset subcommands (`overview`, `budget`, `full`) to run complete experiment suites with a single command.
...
```bash
cd methods/PyramidKV/experiments/scripts
...
- PyramidKV-specific: `--pyram_beta`, `--kernel_size`, `--pooling`
...
- Same as LongBench above (`--max_capacity_prompts`, `--pyram_beta`, `--kernel_size`, `--pooling`)
...
bash run_accuracy.sh --dataset ruler --model qwen --cache_sizes "128" --ruler_seq_lengths "4096" --max_samples 1 --ruler_num_samples 1
...
```bash
bash methods/PyramidKV/experiments/LongBench-v2/run.sh
```
...
```bash
bash methods/PyramidKV/experiments/GSM8K/run.sh
```
...
Located at `methods/PyramidKV/experiments/efficiency/`; not included in `run_accuracy.sh`.
...
Located at `methods/PyramidKV/experiments/efficiency/`. Adapted from the efficiency code — **measures GPU memory only**.
...
Located at `methods/PyramidKV/experiments/recall/`; not included in `run_accuracy.sh`.
...
Located at `methods/PyramidKV/experiments/time/`; measures **sub-component latencies** via CUDA Events.
