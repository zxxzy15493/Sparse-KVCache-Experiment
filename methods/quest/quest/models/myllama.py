...
from quest.models.myQuestAttention import QuestAttention
...
class LlamaDecoderLayer(nn.Module):
  ...
  def forward(
    ...
    """
    phase = "prefill" if hidden_states.shape[1] > 1 else "decode"
    layer_idx = self.self_attn.layer_idx
    global_time.stage_begin(f"{phase}_pre_ffn", layer_idx)
    ...
    hidden_states = residual + hidden_states
    global_time.stage_end(f"{phase}_post_ffn", layer_idx)
