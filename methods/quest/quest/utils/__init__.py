...
def append_kv(
  ...
  seq_len = k.size(0)
  if seq_len > 1:
    global_time.stage_begin("prefill_build_index", layer_idx)
    try:
      ...
      global_time.stage_end("prefill_build_index", layer_idx)
  else:
    global_time.stage_begin("decode_write_cache", layer_idx)
    try:
      ...
      global_time.stage_end("decode_write_cache", layer_idx)
...
def decode_estimate(
  ...
  metadata = iController.metadata_cache.buf_layer(layer_idx)
  if iController.num_heads != iController.num_kv_heads:
    repeat_factor = iController.num_heads // iController.num_kv_heads
    head_dim = 2 if iController.layout == TensorLayout.HND else 3
    metadata = metadata.repeat_interleave(repeat_factor, dim=head_dim)
  f(
    q,
    o,
    metadata,
