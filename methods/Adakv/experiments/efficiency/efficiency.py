...
# ensure headkv module importable
...
def parse_args():
	parser = argparse.ArgumentParser(description="HeadKV efficiency benchmark")
	parser.add_argument("--model", type=str, default=None, help="model path")
	parser.add_argument(
		"--models",
		nargs="+",
		default=None,
		help="llama3.1-8b-128k qwen2.5-7b-instruct",
	)
	...
	parser.add_argument("--method", type=str, required=True, help="AdativeKV")
...
def measure_efficiency(model, tokenizer, text, args):
	...
	if "llama" in model_name_lower or "qwen" in model_name_lower or "mistral" in model_name_lower:
		# Large-context runs can OOM on the lm_head if we keep full vocab logits.
		# Only keep the last token's logits during both prefill and decode.
	...
	for i in range(measure_count):
		...
		print(f"[Measured Round {i}] TTFT: {row['ttft']:.6f} s | TPOT: {tpot_show} s | Latency: {row['latency']:.6f} s")
...
def main():
	...
	for model_name in normalize_model_list(args):
		...
		for input_max_token in normalize_input_lengths(args):
			...
			results, summary = measure_efficiency(model, tokenizer, text, run_args)

			out_file = os.path.join(
				run_dir,
				f"efficiency_{model_args.method}_{model_args.max_capacity_prompts}_{input_max_token}_{model_args.max_new_tokens}.jsonl",
			)
			...
			print(f"Avg TTFT: {summary['avg_ttft']} | Avg TPOT: {summary['avg_tpot']} | Avg Latency: {summary['avg_latency']}")
			print("=" * 80)
