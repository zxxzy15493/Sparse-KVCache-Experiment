...
# === original attention forward clone + recall hooks ===
...
def _set_config_attrs(model, config_params):
...
def enable_headkv_recall(model, method="adativekv", check_recall=False, **config):
    """Enable HeadKV with recall tracking (Adakv only).
    ...
    """
    method_name = "AdativeKV"

    # Step 1: set config attrs so init_headkv can read them
    _set_config_attrs(model, config)
