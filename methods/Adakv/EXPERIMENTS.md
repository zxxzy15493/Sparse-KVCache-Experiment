...
Adakv's **accuracy evaluation** is integrated into the project's unified evaluation framework via `run_accuracy.sh`, which provides preset subcommands (`overview`, `budget`, `full`) to run complete experiment suites with a single command.
...
```bash
cd methods/Adakv/experiments/scripts
...
- `--max_capacity_prompts` (budget): Controls how many KV cache entries are kept per head. Default overview=1024, budget sweep=128/256/512/1024.
- `--head_choice`: `random` (AdativeKV)
...
```bash
cd methods/Adakv/experiments/scripts
# e.g. LongBench: model=llama, budget=128/1024, 1 sample
bash run_accuracy.sh --dataset longbench --model llama --cache_sizes "128 1024" --max_samples 1
...
- Same as LongBench above (`--max_capacity_prompts`, `--head_choice`)
...
```bash
cd methods/Adakv/experiments/scripts
# e.g. RULER: model=qwen, budget=128, seq_length=4096/65536, 1 sample
bash run_accuracy.sh --dataset ruler --model qwen --cache_sizes "128" --ruler_seq_lengths "4096 65536" --ruler_num_samples 1
...
```bash
bash methods/Adakv/experiments/LongBench-v2/run.sh
```

**Experimental parameters — LongBench-v2:**
- `--max_capacity_prompts`: Default 4096
- `--head_choice`: 1 (random, AdativeKV) 
- `--window_size`: 32
- `--kernel_size`: 7, `--pooling`: maxpool, `--floor`: 0.2
- `--cot`: Enables chain-of-thought prompting
...
```bash
bash Sparse-KVCache-Experiment/methods/Adakv/experiments/GSM8K/run.sh
```

**Experimental parameters — GSM8K:**
- `--max_capacity_prompts`: 360 (fixed budget for this benchmark)
- `--num_shots`: 8 (few-shot examples)
- `--max_new_tokens`: 10000 
- Methods: AdativeKV (head_choice=random)

---

## 2. Efficiency (Latency and GPU Memory)

Located at `methods/Adakv/experiments/efficiency/`; not included in `run_accuracy.sh`.
...
- Prefill + decode latency measured separately

---

## 3. Memory (GPU Memory Measurement)

Located at `methods/Adakv/experiments/efficiency/`. Adapted from the efficiency code — **measures GPU memory only**.
...
Located at `methods/Adakv/experiments/recall/`; not included in `run_accuracy.sh`.
...
Located at `methods/Adakv/experiments/time/`; measures **sub-component latencies** via CUDA Events.
