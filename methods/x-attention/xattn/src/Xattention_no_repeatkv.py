...
def _threshold_for_head_range(threshold, head_start: int, head_end: int, num_q_heads: int, num_kv_heads: int):
  if not isinstance(threshold, torch.Tensor):
    return threshold

  flat_threshold = threshold.reshape(-1)
  if flat_threshold.numel() == num_q_heads:
    return flat_threshold[head_start:head_end]
  if flat_threshold.numel() == num_kv_heads:
    num_key_value_groups = num_q_heads // num_kv_heads
    kv_head = head_start // num_key_value_groups
    return flat_threshold[kv_head].expand(head_end - head_start)
  return threshold


def xattn_estimate_no_repeatkv(
  query_states: torch.Tensor,
  key_states: torch.Tensor,
  block_size,
  stride,
  ...
  batch_size, num_q_heads, _, _ = query_states.shape
  _, num_kv_heads, k_len, head_dim = key_states.shape

  if num_q_heads == num_kv_heads:
    return xattn_estimate(
      query_states,
      key_states,
  ...
  for kv_head in range(num_kv_heads):
    head_start = kv_head * num_key_value_groups
    head_end = head_start + num_key_value_groups
    query_group = query_states[:, head_start:head_end, :, :]
    key_group = key_states[:, kv_head : kv_head + 1, :, :].expand(
      batch_size, num_key_value_groups, k_len, head_dim
    )
    group_threshold = _threshold_for_head_range(
      threshold, head_start, head_end, num_q_heads, num_kv_heads
    )
    ...
    attn_sum_groups.append(attn_sums)
    simple_mask_groups.append(simple_masks)

  return torch.cat(attn_sum_groups, dim=1), torch.cat(simple_mask_groups, dim=1)
...
def selected_attn_mass_from_blockmask(
  ...
  for b in range(B):
    for h in range(Hq):
      ...
      kv_h = h if Hk == Hq else h // num_key_value_groups
      K = key_states[b, kv_h].to(torch.float32) # (k_len,D)
  ...
  outdir = Path("efficiency/topk-rate")
  outdir.mkdir(parents=True, exist_ok=True)
...
def Xattention_prefill(
  ...
  attn_sums, approx_simple_mask = xattn_estimate_no_repeatkv(
    query_states,
    key_states,
    block_size=block_size,
    stride=stride,
  ...
  global blocks_num
  if task!=" ":
    blocks_num.append(avg_blocks_per_query.item())
  if len(blocks_num)==28 and model_name=="Qwen" and task!=" ":
    record_block(model_name,task,k_len,block_size)
    blocks_num=[]
  ...
  value_states = value_states.transpose(1, 2).view(k_len, num_kv_heads, head_dim)
  ...
  attn_output = attn_output.view(batch_size, q_len, num_q_heads, head_dim).transpose(
    1, 2
  )
  ################################
