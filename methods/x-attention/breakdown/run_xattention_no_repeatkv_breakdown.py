...
from breakdown.xattention_no_repeatkv_breakdown_core import (
  XattentionNoRepeatKVBreakdownProfiler,
  write_csv,
)
...
def parse_args() -> argparse.Namespace:
  parser = argparse.ArgumentParser(
    description="x-attention no-repeatkv component-level timing breakdown."
  )
  ...
  parser.add_argument("--output_dir", type=str, default="./breakdown_no_repeatkv_core_attn_results")
  parser.add_argument(
    "--output_folder_name",
    "--output_subdir",
    ...
    help=(
      ...
      "Defaults to <family>-xattn-no-repeatkv-stride<stride>-<threshold_tag>."
    ),
  )
...
def build_output_paths(args: argparse.Namespace, family: str, input_len: int):
  ...
  default_folder = f"{family}-xattn-no-repeatkv-stride{args.stride}-{threshold_tag}"
...
def run_one_profile(
  ...
  max_new_tokens: int,
  profiler: XattentionNoRepeatKVBreakdownProfiler,
):
  generated = []
...
def main() -> None:
  args = parse_args()
  assert torch.cuda.is_available(), "x-attention no-repeatkv breakdown requires CUDA."
  ...
  for input_len in target_lengths:
    ...
    rows = []
    print(f"[RUNNING] family={family}, input_len={input_len}, runs={args.num_runs}, no_repeatkv=True")
