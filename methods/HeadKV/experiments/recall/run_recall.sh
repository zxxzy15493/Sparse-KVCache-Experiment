    ...
    for model_key in "${LB_MODELS[@]}"; do
        ...
        for budget in "${LB_BUDGETS[@]}"; do
            for ds in "${LB_DATASETS[@]}"; do
                config_path="config/${method}/${model_key}_c${budget}_w${WINDOW}.json"
                echo "============================================================"
                echo "  [A][${method}] Model: $model_key | Dataset: $ds | Budget: $budget"
                echo "============================================================"
                python recall.py \
                    --model "$model_path" \
                    --dataset "$LB_DATA" \
                ...
                if [ $? -eq 0 ]; then
                    echo "  [OK] [${method}] $model_key / $ds / budget=$budget done."
                else
                    echo "  [FAIL] [${method}] $model_key / $ds / budget=$budget failed!"
                fi
                echo ""
            done
        done
    ...
    for model_key in "${RULER_MODELS[@]}"; do
        ...
        for budget in "${RULER_BUDGETS[@]}"; do
            for task in "${RULER_TASKS[@]}"; do
                config_path="config/${method}/${model_key}_c${budget}_w${WINDOW}.json"
                echo "============================================================"
                echo "  [B][${method}] Model: $model_key | Task: $task | Budget: $budget"
                echo "============================================================"
                python recall.py \
                    --model "$model_path" \
                    ...
                    --compress_args_path "$config_path"
                if [ $? -eq 0 ]; then
                    echo "  [OK] [${method}] $model_key / $task / budget=$budget done."
                else
                    echo "  [FAIL] [${method}] $model_key / $task / budget=$budget failed!"
                fi
                echo ""
            done
        done
...
echo ""
echo "=============================================="
echo " All HeadKV recall experiments finished!"
echo "=============================================="
