...
def _iter_prediction_files(root_dir):
    for dirpath, _, filenames in os.walk(root_dir):
        for filename in filenames:
            if filename.endswith(".jsonl") or filename.endswith(".json"):
                lower = filename.lower()
                if lower == "result.json" or lower.endswith("metrics.json"):
                    continue
                yield os.path.join(dirpath, filename)

def _infer_dataset_name(file_path):
    base = os.path.basename(file_path)
    dataset = base.split('.')[0]
    if dataset in dataset2metric:
        return dataset
    parent = os.path.basename(os.path.dirname(file_path))
    if parent in dataset2metric:
        return parent
    return dataset
...
if __name__ == '__main__':
    ...
    all_files = list(_iter_prediction_files(path))
    print("Evaluating on:", [os.path.relpath(p, path) for p in all_files])
    for file_path in all_files:
        predictions, answers, lengths = [], [], []
        dataset = _infer_dataset_name(file_path)
        bad_lines = 0
        first_bad = None
        ...
        if bad_lines:
            print(
                f"[WARN] {os.path.relpath(file_path, path)} skipped {bad_lines} invalid lines"
                f" (first at line {first_bad})"
            )
        if not predictions:
            continue
        if args.e:
