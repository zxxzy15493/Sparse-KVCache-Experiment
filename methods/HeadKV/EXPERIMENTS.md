...
HeadKV's **accuracy evaluation** is integrated into the project's unified evaluation framework via `run_accuracy.sh`, which provides preset subcommands (`overview`, `budget`, `full`) to run complete experiment suites with a single command.
...
```bash
cd methods/HeadKV/experiments/scripts
...
- Same as LongBench above (`--max_capacity_prompts`)
...
```bash
bash methods/HeadKV/experiments/LongBench-v2/run.sh
```

**Experimental parameters — LongBench-v2:**
- `--max_capacity_prompts`: Default 4096
- `--kernel_size`: 7, `--pooling`: maxpool
...
```bash
bash methods/HeadKV/experiments/GSM8K/run.sh
```
...
Located at `methods/HeadKV/experiments/efficiency/`; not included in `run_accuracy.sh`.
...
Located at `methods/HeadKV/experiments/efficiency/`. Adapted from the efficiency code — **measures GPU memory only**.
...
Located at `methods/HeadKV/experiments/recall/`; not included in `run_accuracy.sh`.
...
Located at `methods/HeadKV/experiments/time/`; measures **sub-component latencies** via CUDA Events.
