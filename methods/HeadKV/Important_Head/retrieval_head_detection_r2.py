...
from HeadKV.Important_Head.metrics import qa_f1_score
import ipdb
...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
        ...
        self.haystack_dir_list = [f"{haystack_dir}/part{i}" for i in range(1, len(self.needle_list) + 1)]
        ...
        self.answers_list = [l.get("answer", l["real_needle"]) for l in needles_and_stacks]
        self.task = task
        ...
        self.is_glm = ("glm" in self.model_version.lower() or "chatglm" in self.model_version.lower())
        ...
        if self.is_glm:
            ...
            self.model_to_test = chatglm_cls.from_pretrained(
                model_name,
                torch_dtype="auto",
                device_map='auto',
                trust_remote_code=True,
            ).eval()
            self._patch_glm_core_attention()
            config = self.model_to_test.config
            self.layer_num, self.head_num = config.num_hidden_layers, config.num_attention_heads
        elif "Mixtral" in self.model_version:
            from HeadKV.Important_Head.source.modeling_mixtral import MixtralForCausalLM
            ...
            from HeadKV.Important_Head.source.modeling_mistral import MistralForCausalLM
            config = AutoConfig.from_pretrained(model_name, trust_remote_code=True)
            self.layer_num, self.head_num = config.num_hidden_layers, config.num_attention_heads
            self.model_to_test = MistralForCausalLM.from_pretrained(
                    model_name,torch_dtype="auto",device_map='auto',use_flash_attention_2="flash_attention_2",trust_remote_code=True,
                ).eval()
        elif "DeepSeek" in self.model_version:
            from HeadKV.Important_Head.source.modeling_qwen2 import Qwen2ForCausalLM
            config = AutoConfig.from_pretrained(model_name, trust_remote_code=True)
            self.layer_num, self.head_num = config.num_hidden_layers, config.num_attention_heads
            ...
            from HeadKV.Important_Head.source.modeling_llama import LlamaForCausalLM
            config = AutoConfig.from_pretrained(model_name, trust_remote_code=True)
            self.layer_num, self.head_num = config.num_hidden_layers, config.num_attention_heads
    ...
    def _get_glm_layers(self):
        if hasattr(self.model_to_test, 'transformer') and hasattr(self.model_to_test.transformer, 'encoder'):
            return self.model_to_test.transformer.encoder.layers
        if hasattr(self.model_to_test, 'model') and hasattr(self.model_to_test.model, 'layers'):
            return self.model_to_test.model.layers
        return []

    def _patch_glm_core_attention(self):
        layers = self._get_glm_layers()
        if len(layers) == 0:
            return
        for layer in layers:
            core_attn = layer.self_attention.core_attention
            if getattr(core_attn, '_headkv_capture_patched', False):
                continue
            original_forward = core_attn.forward

            def forward_with_capture(this, query_layer, key_layer, value_layer, attention_mask, *args, **kwargs):
                try:
                    q = query_layer.float()
                    k = key_layer.float()
                    scores = torch.matmul(q, k.transpose(-1, -2))
                    norm_factor = getattr(this, 'norm_factor', None)
                    if norm_factor is not None:
                        scores = scores / float(norm_factor)
                    if attention_mask is not None:
                        mask = attention_mask.to(torch.bool) if attention_mask.dtype != torch.bool else attention_mask
                        scores = scores.masked_fill(mask, float("-inf"))
                    attn_probs = torch.softmax(scores, dim=-1).to(query_layer.dtype)
                    this._last_attention_probs = attn_probs.detach()
                except Exception:
                    this._last_attention_probs = None
                return original_forward(query_layer, key_layer, value_layer, attention_mask, *args, **kwargs)

            core_attn.forward = types.MethodType(forward_with_capture, core_attn)
            core_attn._headkv_capture_patched = True

    def _get_glm_attentions(self):
        layers = self._get_glm_layers()
        if len(layers) == 0:
            return None
        attns = [getattr(layer.self_attention.core_attention, '_last_attention_probs', None) for layer in layers]
        if any(attn is None for attn in attns):
            return None
        return attns
    ...
    def retrieval_calculate(self, attention_maxtrix,retrieval_score, inp, step_token,topk=1, answer_tokens_num=1):
        for layer_idx in range(self.layer_num):
            for head_idx in range(self.head_num):
                attn_vec = attention_maxtrix[layer_idx][0][head_idx][-1]
                seq_len = attn_vec.shape[-1]
                start = max(0, min(self.needle_start, seq_len))
                end = max(start + 1, min(self.needle_end, seq_len))
                span_mass = attn_vec[start:end].mean().item()
                retrieval_score[layer_idx][head_idx][0] += span_mass
                retrieval_score[layer_idx][head_idx][1] += 1
    ...
    def decode(self, q_outputs, inp, decode_len, block_list=None):
        output, retrieval_score = [], [[[0, 0] for _ in range(self.head_num)] for _ in range(self.layer_num)]
        answer_tokens = self.enc.encode(self.answer, add_special_tokens=False)
        past_kv = q_outputs.past_key_values
        for step_i in range(decode_len):
            inp = inp.view(1, 1).to(next(self.model_to_test.parameters()).device)
            if self.is_glm:
            ...
            attn_matrix = self._get_glm_attentions() if self.is_glm else outputs.attentions
            if attn_matrix is not None:
                self.retrieval_calculate(attn_matrix, retrieval_score, inp, step_token, answer_tokens_num=len(answer_tokens))
            if step_token=='<0x0A>' or inp.item()==144 or inp.item() == self.model_to_test.config.eos_token_id or len(output) >= len(answer_tokens): break
    ...
    def find_needle_idx(self, needle):
        ...
        print('original needle: ' + self.enc.decode(needle_ids, skip_special_tokens=False))
        span_len = len(needle_ids)
        max_overlap = 0.
        start = end = 0
        if len(self.enc.encode('')) != 0:
            for i in range(len(self.prompt_ids)):            
                if self.prompt_ids[i] == self.enc.encode('')[0]:
                    # ipdb.set_trace()
                    token_span = self.prompt_ids[i + 1: i + span_len + 1]
        ...
        print('found needle: ' + self.enc.decode(self.prompt_ids[start:end], skip_special_tokens=False))
        return start, end
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        template = f"Based on the content of the book, Question: {self.retrieval_question}\nAnswer:"
        ...
        input_ids = input_ids.to(next(self.model_to_test.parameters()).device)
        self.needle_start, self.needle_end = self.find_needle_idx(self.real_needle)
        with torch.no_grad():
            # Use self.model_to_test.model to avoid calculating logits for the entire prefix, which causes OOM
            if self.is_glm:
        ...
        # if score > 50:
        score = qa_f1_score(response, self.answer)*100
        ...
        if self.save_contexts:
            ...
            if not os.path.exists(f'contexts_{self.task}/{self.model_version}'):
                os.makedirs(f'contexts_{self.task}/{self.model_version}')

            with open(f'contexts_{self.task}/{self.model_version}/{context_file_location}_context.txt', 'w') as f:
                f.write(context)
    ...
    def encode_text_to_tokens(self, text):
        if self.model_provider in ["OpenAI", "LLaMA", "LLAMA3", "Mistral", "GLM", "Qwen2"]:
    ...
    def insert_needle(self, context, depth_percent, context_length):
        ...
        if depth_percent == 100:
            ...
            elif(self.model_provider == "Qwen2"): period_tokens = [13, 113, 144]
    ...
    def start_test(self, args):
        for ni in range(len(self.needle_list)):
            ...
            self.run_test(args)
        # if os.path.exists(f"head_score/test.json"):
        #     with open(f"./head_score/test.json", "r") as file:
        #         head_counter = json.loads(file.readline())
        #     for k,v in head_counter.items():
        #         self.head_counter[k] += v
        # with open(f"head_score/test.json", 'w') as f:
        #     json.dump(self.head_counter, f)
        if os.path.exists(f"head_score/{self.model_version}_{self.task}.json"):
            with open(f"./head_score/{self.model_version}_{self.task}.json", "r") as file:
                head_counter = json.loads(file.readline())
            for k,v in head_counter.items():
                self.head_counter[k] += v
        with open(f"head_score/{self.model_version}_{self.task}.json", 'w') as f:
            json.dump(self.head_counter, f)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--haystack_dir', type=str, default='./haystack_for_detect')
