...
from .minfer_patterns import qwen_25_7B_Instruct_1M

class DeepSeekLayer:
    """
    A class representing the DeepSeekQwenModel layer.
    """
    ...
    def init_layer(self, hf_qwen_layer):
        ...
        self.bq = hf_qwen_layer.self_attn.q_proj.bias.detach() #mark 
        self.bk = hf_qwen_layer.self_attn.k_proj.bias.detach() #mark 
        self.bv = hf_qwen_layer.self_attn.v_proj.bias.detach() #mark 
        self.wqkv = torch.cat((self.wq, self.wk, self.wv), dim=0).to(self.device, non_blocking=True)
        self.bqkv = torch.cat((self.bq, self.bk, self.bv), dim=0).to(self.device, non_blocking=True) #mark
...
class DeepSeekQwenModel(LLM):
    """
    A class representing the DeepSeekQwenModel.
    """
    ...
    def __init__(
        ...
        dtype: torch.dtype,
        device_map: str,
        fixed_output_length: int = -1, # The fixed output length for decoding. If -1, it will not intervene the output length.
        RECALL: bool = False,
        ...
        self.base = self.config.rope_theta #mark
        ...
        self.best_patterns = qwen_25_7B_Instruct_1M

        self.init_model()

    
    def _set_cos_sin_cache(self): #mark
    ...
    def init_model(self):
        ...
        if self.device_map != "auto":   # single GPUs
            ...
            for idx, hf_qwen_layer in enumerate(hf_qwen.model.layers):
                deepseek_layer = DeepSeekLayer(idx, device=self.device_map)
                deepseek_layer.init_layer(hf_qwen_layer)
                self.layers.append(deepseek_layer)
            ...
            for ldx, hf_qwen_layer in enumerate(hf_qwen.model.layers):
                deepseek_layer = DeepSeekLayer(ldx, device=self.layer_mapping[str(ldx)])
                deepseek_layer.init_layer(hf_qwen_layer)
                self.layers.append(deepseek_layer)
