...
class QwenModel(LLM):
    ...
    def __init__(
        ...
        super().__init__(model_name, max_length, dtype, device_map, RECALL, budget)
        ...
        self.yarn_factor = 4  #mark       # # qwen2.5 use yarn in context length larger than 32768
    ...
    def prefill_attention(self, query_states, key_states, value_states, layer_idx):
        if self.prefill_method == 'Full_Flash_Attn':
            ...
            attn_out = prefill_minfer(query_states, key_states, value_states, self.best_patterns[layer_idx], RECALL=self.RECALL)
        else:
            raise ValueError(f"Unsupported attention type: {self.attention_type}")
        return attn_out
