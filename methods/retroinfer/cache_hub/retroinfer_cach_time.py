...
import time 
from model_hub_time.TimeManager import timeManager
...
class retroinfer_cache_time(KV_Cache):
    """
    ...
    def prefill_update_kv_cache(self, query_states, key_states, value_states, layer_idx, batch_idx): 
        ...
        with torch.cuda.stream(self.copystream):
            ...
            timeManager.unload_start_event[layer_idx].record(stream=self.copystream)
            ...
            timeManager.unload_end_event[layer_idx].record(stream=self.copystream)
        ...
        timeManager.construct_start_event[layer_idx].record()
        ...
        timeManager.construct_end_event[layer_idx].record()
        construct_end = time.perf_counter()
        timeManager.construct_time += (construct_end - construct_start)
    ...
    def sync(
        ...
        """
        sync_inner_start = time.perf_counter()
        timeManager.kvcache_sync_start_event[layer_idx].record()
        ...
        timeManager.kvcache_sync_end_event[layer_idx].record()
        sync_inner_end = time.perf_counter()
        timeManager.kvcache_sync_time += (sync_inner_end - sync_inner_start)
    ...
    def compute(self, queries, layer_idx):
        ...
        timeManager.compute_method_start_event[self.layer_num * timeManager.decode_step + layer_idx].record()
        static_len = self.static_pattern_total if layer_idx == self.layer_num - 1 else self.static_pattern_total + 1

        # retrieve_start = time.perf_counter()
        timeManager.retrieve_start_event[self.layer_num * timeManager.decode_step + layer_idx].record()
        ...
        timeManager.retrieve_end_event[self.layer_num * timeManager.decode_step + layer_idx].record()
        # retrieve_end = time.perf_counter()
        # self.retrieve_time += (retrieve_end - retrieve_start)

        # estimation zone computation
        if self.es_cluster_num > 0:
            # esitimate_start = time.perf_counter()
            timeManager.esitimate_load_start_event[self.layer_num * timeManager.decode_step + layer_idx].record()
            ...
            timeManager.esitimate_load_end_event[self.layer_num * timeManager.decode_step + layer_idx].record()
            # esitimate_end = time.perf_counter()
            # self.esitimate_load_time += (esitimate_end - esitimate_start)

            esitimate_attn_start = time.perf_counter()
            timeManager.esitimate_attn_start_event[self.layer_num * timeManager.decode_step + layer_idx].record()
            ...
            timeManager.esitimate_attn_end_event[self.layer_num * timeManager.decode_step + layer_idx].record()
            esitimate_attn_end = time.perf_counter()
            timeManager.esitimate_attn_time += (esitimate_attn_end - esitimate_attn_start)
        else:
            es_out, es_lse = None, None
        
        load_start = time.perf_counter()
        timeManager.load_start_event[self.layer_num * timeManager.decode_step + layer_idx].record()
        ...
        timeManager.load_end_event[self.layer_num * timeManager.decode_step + layer_idx].record()
        load_end = time.perf_counter()
        timeManager.load_time += (load_end - load_start)


        attn_start = time.perf_counter()
        timeManager.attn_start_event[self.layer_num * timeManager.decode_step + layer_idx].record()
        ...
        timeManager.attn_end_event[self.layer_num * timeManager.decode_step + layer_idx].record()
        attn_end = time.perf_counter()
        timeManager.attn_time += (attn_end - attn_start)
        
        # update_start = time.perf_counter()
        timeManager.update_start_event[self.layer_num * timeManager.decode_step + layer_idx].record()
        ...
        timeManager.update_end_event[self.layer_num * timeManager.decode_step + layer_idx].record()
        # update_end = time.perf_counter()
        # self.update_time += (update_end - update_start)
        
        compute_end = time.perf_counter()
        timeManager.compute_method_end_event[self.layer_num * timeManager.decode_step + layer_idx].record()  
        timeManager.compute_method_time += (compute_end - compute_start)
