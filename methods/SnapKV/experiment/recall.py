...
from snapkv.monkeypatch.snapkv_recall import enable_snapkv_recall, stats_registry, init_new_sample_registry
...
def build_output_path(model_name, dataset_name, dataset_path, compress_args=None):
    base_output = "./recall_results"
    
    if compress_args and 'max_capacity_prompts' in compress_args:
        caps = compress_args['max_capacity_prompts']
...
def get_pred_single_gpu(data, max_length, max_gen, prompt_format, dataset, dataset_name, model_name, model2path, output_path, compress_args=None):
    ...
    if compress_args is not None:
        enable_snapkv_recall(model, check_recall=True, **compress_args)
...
if __name__ == '__main__':
    ...
    output_path = build_output_path(model_name, dataset_name, dataset_path, compress_args)
    ...
    get_pred_single_gpu(data_all, max_length, max_gen, prompt_format, dataset, dataset_name, model_name, model2path, output_path, compress_args)

    stats_path_csv = output_path.replace(".jsonl", "_attn_ratios.csv")
    print(f"\nFlattening and saving diagnostic results to CSV...")
