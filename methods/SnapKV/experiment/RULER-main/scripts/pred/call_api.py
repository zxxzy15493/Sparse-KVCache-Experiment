...
parser.add_argument("--enable_snapkv", action="store_true", help="Enable SnapKV compression")
parser.add_argument("--compress_args_path", type=str, default=None, help="Path to the compress args json file")
parser.add_argument("--check_recall", action="store_true", help="Enable recall diagnostic")
...
print("comress_args_path:", args.compress_args_path)
def get_llm(tokens_to_generate):
    if args.server_type == 'trtllm':
        ...
        if args.check_recall:
            ...
            enable_snapkv=args.enable_snapkv,
            compress_args_path=args.compress_args_path,
...
def main():
    ...
    if args.server_type == 'hf' and args.enable_snapkv and args.check_recall:
        try:
            from snapkv.monkeypatch.snapkv_recall import init_new_sample_registry, stats_registry
            import csv
            import numpy as np
            ...
            print(f"\nRULER Task '{args.task}' (SnapKV) done. Processing metric matrices...")
                        ...
                        for l_idx, matrix in step.items():
                            ...
                            if matrix.ndim == 2 and matrix.shape[1] == 3:
                                for h_idx in range(matrix.shape[0]):
                                    r100 = float(matrix[h_idx, 0])
                                    rk = float(matrix[h_idx, 1])
                                    ratio = float(matrix[h_idx, 2])

                                    writer.writerow([
                                        s_idx, step_idx, l_idx, h_idx,
                                        f"{r100:.6f}", f"{rk:.6f}", f"{ratio:.6f}"
                                    ])
                                    row_count += 1

                                    sum_recall_100 += r100
                                    sum_recall_k += rk
                                    sum_attn_ratio += ratio
            ...
            print(f"Binary backup .npz file safely archived.")

            if total_element_count > 0:
                avg_r100 = (sum_recall_100 / total_element_count) * 100
                avg_rk = (sum_recall_k / total_element_count) * 100
                avg_ratio = (sum_attn_ratio / total_element_count) * 100

                print("\n" + "="*60)
                print(f"SNAPKV KV CACHE DIAGNOSTIC FINAL REPORT ({args.task})  📊")
                print(f"Raw CSV log archived at: {stats_path_csv}\n")
            else:
                print("\nNo tokens triggered SnapKV eviction diagnostic.\n")

        except Exception as e:
            print(f"Failed to compile dynamic diagnostic report: {e}")
            traceback.print_exc()
