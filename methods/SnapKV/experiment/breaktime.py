...
import snapkv.monkeypatch.snapkv_time as SK

def parse_args(args=None):
    p = argparse.ArgumentParser()
    p.add_argument('--model', type=str, required=True)
    p.add_argument('--window_size', type=int, default=32)
    p.add_argument('--max_capacity_prompt', type=int, default=1024)
    p.add_argument('--kernel_size', type=int, default=7)
    p.add_argument('--pooling', type=str, default='avgpool')
...
def build_output_path(model_name, dataset_name, dataset_path, max_length, cap=None):
    base = "./time_profile_results"
    budget_dir = f"budget{cap}" if cap else "budget_full"
...
CSV_HEADER = [
    ...
    "decode_avg(s)", "decode_writeback_avg(s)", "decode_attn_avg(s)", "decode_ffn_avg(s)",
]
REGISTRY_KEYS = [
    "run_idx",
    "prefill_total", "construct", "prefill_writeback", "prefill_attn", "prefill_ffn",
    "decode_avg", "decode_writeback_avg", "decode_attn_avg", "decode_ffn_avg",
]

def collect_times(model, max_gen):
    construct = 0.0; prefill_writeback = 0.0; prefill_attn = 0.0
    decode_writeback_total = 0.0; decode_attn_total = 0.0
...
def collect_global_times(max_gen):
    SK.global_prefill_start_event.synchronize()
    SK.global_prefill_end_event.synchronize()
    prefill_total = SK.global_prefill_start_event.elapsed_time(SK.global_prefill_end_event) / 1000.0
    SK.global_decode_start_event.synchronize()
    SK.global_decode_end_event.synchronize()
    decode_total = SK.global_decode_start_event.elapsed_time(SK.global_decode_end_event) / 1000.0
    return {"prefill_total": prefill_total, "decode_avg": decode_total / max_gen if max_gen > 0 else 0.0}
...
if __name__ == '__main__':
    ...
    SK.enable_snapkv_time(model, window_sizes=args.window_size,
                           max_capacity_prompts=args.max_capacity_prompt,
                           kernel_sizes=args.kernel_size, pooling=args.pooling)
    ...
    for max_length in [4096, 65536]:
        ...
        for run_idx in range(args.num_runs):
            ...
            SK.global_decode_end_event.record()
            ...
            decode_ffn_avg = global_["decode_avg"] - local["decode_writeback_avg"] - local["decode_attn_avg"]
            ...
            SK.breaktime_registry.append(entry)
