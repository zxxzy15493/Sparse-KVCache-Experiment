...
def read_manifest(path: Union[str, Path], encoding: str = "utf-8") -> list[dict]:
    """
     JSONL manifest  list[dict].
     JSON 
    """
    path = Path(path)
    data: list[dict] = []
    ...
    with path.open("r", encoding=encoding) as f:
        for lineno, line in enumerate(f, start=1):
            ...
            if not isinstance(obj, dict):
                raise ValueError(
                    f"Each line in manifest must be a JSON object, "
                    f"but got {type(obj).__name__} at line {lineno} in {path}"
                )
    ...
    with path.open(mode, encoding=encoding) as f:
        for idx, item in enumerate(items, start=1):
            if not isinstance(item, dict):
                raise ValueError(
                    f"write_manifest expects dict items, "
...
def find_prediction_file(task: str, jsonl_files: list[str]):
    task_lower = task.lower()
    exact_file = f"{task}.jsonl"

    # exact match first
    for file in jsonl_files:
        if file.lower() == exact_file.lower():
            return file

    # keyword match second
    candidates = []
    pattern = re.compile(rf'(?<![A-Za-z0-9]){re.escape(task_lower)}(?![A-Za-z0-9])')
    ...
    candidates.sort(
        key=lambda x: (
            0 if Path(x).stem.lower().startswith(task_lower) else 1,
            len(x)
        )
    )

    if len(candidates) > 1:
        print(f"[Warning] Multiple files matched task '{task}': {candidates}. Use '{candidates[0]}'")
...
def main():
    ...
    for task, config in TASKS.items():
        ...
        matched_file = find_prediction_file(task, jsonl_files)
        if matched_file is None:
            print(f"Prediction file for task '{task}' is not found.")
            continue

        print(f"Evaluate task {task} with file {matched_file}...")
        task_score, task_nulls, predicts, indices = run_evaluation_per_task(
            predictions_file=os.path.join(args.data_dir, matched_file),
            task_config=config,
        )
        eval_results[task] = {
            'score': task_score,
