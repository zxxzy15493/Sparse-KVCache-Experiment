...
# ==========================================
# ==========================================
class CakeRecallCache:
    def __init__(self, max_capacity_prompt=1024, window_size=32, gamma=200.0):
...
def common_diagnostic_and_evict(self, query_states, key_states, value_states, past_key_value, bsz, q_len, sin, cos):
    ...
    if q_len > 1:
        ...
        if kv_seq_len <= self.kv_cache.cache_size:
            ...
            attention_score = attn_weights[:, :, -window_size:, :]    # [bsz, num_heads, window_size, seq]
            attn_mean = attention_score.mean(dim=-2)                   # [bsz, num_heads, seq]
            attn_var = attention_score.var(dim=-2)                     # [bsz, num_heads, seq]
            attn_cache = attn_mean + self.kv_cache.gamma * attn_var    # [bsz, num_heads, seq]
            ...
            topk_indices = attn_cache.topk(min(keep_capacity, attn_cache.shape[-1]), dim=-1).indices
...
# ==========================================
# ==========================================
def llama_forward_cake_recall(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None, **kwargs):
...
def qwen_forward_cake_recall(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None, **kwargs):
...
# ======================================================================
# ======================================================================
def enable_cake_recall(model, check_recall=False, window_sizes=None, max_capacity_prompts=None, gamma=200.0):
    ...
    if not isinstance(window_sizes, list):
        window_sizes = [window_sizes] * num_layers
    if not isinstance(max_capacity_prompts, list):
        max_capacity_prompts = [max_capacity_prompts] * num_layers
    ...
    for name, module in model.named_modules():
        if (llama_classes and isinstance(module, llama_classes)) or (qwen_classes and isinstance(module, qwen_classes)):
            ...
            module.kv_cache = CakeRecallCache(
            ...
            if llama_classes and isinstance(module, llama_classes):
                module.forward = types.MethodType(llama_forward_cake_recall, module)
            else:
                module.forward = types.MethodType(qwen_forward_cake_recall, module)
            patched_layers += 1

    print(f"CakeKV Recall Patch Enabled. Patched {patched_layers} attention layers.")