...
def minference_prefill_forward(
    ...
    for bdx in range(bsz):
        for head in range(query_states.size(1)):
            ...
            if RECALL:
                attn_output, head_recall, head_weight = minference_prefill_kernel(q, k, v, head, layer_idx, best_pattern, RECALL)
                recall_layer_json["head_recalls"].append(head_recall)
                recall_layer_json["head_weights"].append(head_weight)
            else:
                attn_output = minference_prefill_kernel(q, k, v, head, layer_idx, best_pattern, RECALL)
            output[bdx, head:head+1, :, :] = attn_output
        if RECALL:
            recall_layer_json["avg_head_recall"] = sum(recall_layer_json["head_recalls"]) / len(recall_layer_json["head_recalls"]) if len(recall_layer_json["head_recalls"]) else 0
            recall_layer_json["avg_head_weight"] = sum(recall_layer_json["head_weights"]) / len(recall_layer_json["head_weights"]) if len(recall_layer_json["head_weights"]) else 0
            recall_json.append(recall_layer_json)    
    return output
