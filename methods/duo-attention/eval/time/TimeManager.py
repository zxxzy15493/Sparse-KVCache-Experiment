"""
Unified CUDA Event TimeManager for 12-component measurement (DuoAttention).

Measures:
  Prefill: lat(s) / pattern(s) / attn(s) / ffn(s) / index(s) / write_cache(s)
  Decode:  lat(ms) / attn(ms) / ffn(ms) / retrieve(ms) / write_cache(ms)  (32-step avg)
...
class TimeManager:
    ...
    def _alloc_events(self):
        ...
        # new: qkv_proj, o_proj, write_cache (prefill uses idx < L, decode uses idx L..L*M)
    ...
    # -- Event recording methods (called directly from attention forward) --
    # prefill-only (L-length, use layer_idx directly)
    ...
    # ── properties for _timing_events dict compatibility ──
    @property
    ...
    @property
    def dec_retrieve_start(self): return self._dec_retrieve_start
    @property
    def dec_retrieve_end(self):   return self._dec_retrieve_end
    @property
    def dec_pure_start(self):     return self._dec_pure_start
    @property
    def dec_pure_end(self):       return self._dec_pure_end

    @property
    def qkv_proj_start(self):    return self._qkv_proj_start
    @property
    def qkv_proj_end(self):      return self._qkv_proj_end
    @property
    def o_proj_start(self):      return self._o_proj_start
    @property
    def o_proj_end(self):        return self._o_proj_end
    @property
    def write_cache_start(self): return self._write_cache_start
    @property
    def write_cache_end(self):   return self._write_cache_end
    ...
    def finish_round(self):
        ...
        for l in range(L):
            idx = l  # decode_step == 0 during prefill
            ...
            # attn = pure flash attention only
            prefill_attn += pure_attn
            # ffn = layernorm + mlp + qkv_proj + o_proj + rope/expand/reshape
    ...
    def get_averaged_report(self) -> dict:
        ...
        return {key: sum(r[key] for r in self._rounds) / len(self._rounds) for key in keys}
