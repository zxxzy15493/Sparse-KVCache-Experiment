"""
DuoAttention timing test — 9-component CUDA Event measurement.

Protocol:  4 warmup + 1 measure, all rounds written to report.
Usage:
    cd methods/duo-attention/eval/time
    python run_timing_test.py --model Meta-Llama-3.1-8B-Instruct --sparsity 0.5
"""
...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "duo_attn"))

from TimeManager import time_manager
from models_patch import patch_model_with_timing

MODELS = {"Meta-Llama-3.1-8B-Instruct": "meta-llama/Llama-3.1-8B-Instruct"}
SPARSITIES = [0.5]


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", type=str, default="Meta-Llama-3.1-8B-Instruct", choices=list(MODELS.keys()))
...
def _build_report(rounds: list, warmup: int) -> str:
    sep = "=" * 60
    lines = [sep, "  DuoAttention timing report", sep]
...
def main():
    ...
    subdirs = sorted(glob.glob(f"../../attn_patterns/{args.model}/*/"))
    if not subdirs:
        raise FileNotFoundError(f"No pattern subdirectory found for {args.model}")
    pattern_dir = subdirs[0]
    ...
    from duo_attn.utils import load_attn_pattern
    full_attention_heads, _, _ = load_attn_pattern(pattern_dir)
    enable_duo_attention_eval(model, full_attention_heads, args.sink_size, args.recent_size)
    print(f"DuoAttention enabled (sparsity={args.sparsity}).")
    ...
    print(report_str)
    report_path = f"timing_{args.model}_sp{args.sparsity}_{args.input_max_token}_{args.max_new_tokens}_report.txt"
