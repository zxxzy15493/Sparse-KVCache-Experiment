...
# from ..utils import calculate_duo_attn_recall
...
def llama_duo_attention_forward_one_way_reordered(
    ...
    **kwargs,
    ...
    if q_len == kv_seq_len:
        ...
        attn_output, lse, attn_weights = flash_attn_func(
            query_states,
            key_states,
            value_states,
            causal=True,
        ...
        if attn_weights is not None:
            # attn_weights from flash_attn: [bsz, num_heads, q_len, kv_seq_len] (if return_attn_probs=True)
            # But standard flash_attn might not return 4D weights directly without specific flags or versions.
            # Assuming attn_weights is [bsz, num_heads, q_len, kv_seq_len] here.
            scores_for_recall = attn_weights
        else:
            # Fallback to manual calculation if flash_attn doesn't return weights
            with torch.cuda.amp.autocast(enabled=False):
                # [bsz, q_len, num_heads, head_dim] -> [bsz, num_heads, q_len, head_dim]
                q = query_states.transpose(1, 2).to(torch.float32)
                k = key_states.transpose(1, 2).to(torch.float32)
                ...
                # Apply causal mask
                mask = torch.triu(torch.ones(q_len, kv_seq_len, device=q.device), diagonal=1).bool()
                scores.masked_fill_(mask, float("-inf"))
                attn_weights_manual = torch.softmax(scores, dim=-1)
                scores_for_recall = attn_weights_manual
        
        full_attn_head_mask = self.full_attn_head_mask
        sink_size = self.sink_size
        recent_size = self.recent_size
        num_kv_groups = self.num_key_value_groups
        
        decode_recalls, prefill_recalls, decode_attn_ratios, prefill_attn_ratios = calculate_duo_attn_recall(
            scores_for_recall, 
            full_attn_head_mask, 
            sink_size, 
            recent_size, 
        ...
        if not hasattr(self, "decode_recall_list"): self.decode_recall_list = []
        if not hasattr(self, "prefill_recall_list"): self.prefill_recall_list = []
        if not hasattr(self, "decode_attn_ratio_list"): self.decode_attn_ratio_list = []
        if not hasattr(self, "prefill_attn_ratio_list"): self.prefill_attn_ratio_list = []
        
        self.decode_recall_list.extend(decode_recalls)
        self.prefill_recall_list.extend(prefill_recalls)
        self.decode_attn_ratio_list.extend(decode_attn_ratios)
        self.prefill_attn_ratio_list.extend(prefill_attn_ratios)
        
        if not hasattr(self, "mass_recall_list"):
            self.mass_recall_list = []
        self.mass_recall_list.extend(prefill_recalls)
    else:
...
def llama_duo_attention_forward_one_way_reordered_static(
    ...
    **kwargs,
    ...
    if q_len == kv_seq_len:
        ...
        if not hasattr(self.config, "duo_attn_recall"):
            self.config.duo_attn_recall = []
        
        # Calculate Recall
        # attn_weights shape from flash_attn_func with return_attn_probs=True is usually [bsz, num_heads, q_len, kv_len]
        # For pre-filling q_len == kv_len, we take the last query's attention weights as the global importance
        # Or more accurately, use the mean across query dimension for the pre-fill stage
        if attn_weights is not None:
            gt_weights = attn_weights.mean(dim=2) # [bsz, num_heads, kv_seq_len]
        else:
            # Fallback to manual calculation if flash_attn doesn't return weights
            with torch.cuda.amp.autocast(enabled=False):
                # [bsz, q_len, num_heads, head_dim] -> [bsz, num_heads, q_len, head_dim]
                ...
                scores.masked_fill_(mask, float("-inf"))
                attn_weights_manual = torch.softmax(scores, dim=-1)
                gt_weights = attn_weights_manual.mean(dim=2)
        ...
        recalls = calculate_duo_attn_recall(
            gt_weights, 
            full_attn_head_mask, 
            sink_size, 
            recent_size, 
        ...
        if not hasattr(self, "mass_recall_list"):
            self.mass_recall_list = []
        self.mass_recall_list.extend(recalls)
