...
def parse_args():
    ...
    parser.add_argument(
        "--dataset_name",
        type=str,
        default="duo_attn/Long-Data-Collections/pretrain/pile_sub.jsonl.zst",
    )
...
def full_attention_heads_to_list(full_attention_heads):
    ...
    for h in full_attention_heads:
        try:
            # If this is a DTensor-like object, convert to local tensor first
            if hasattr(h, "to_local"):
                h_local = h.to_local()
            else:
                h_local = h
            arr = h_local.detach().float().cpu().numpy().tolist()
        except Exception:
            try:
                # Fallback: try treating as a torch tensor
                arr = torch.as_tensor(h).detach().float().cpu().numpy().tolist()
            except Exception:
                arr = None
        out.append(arr)
    return out
