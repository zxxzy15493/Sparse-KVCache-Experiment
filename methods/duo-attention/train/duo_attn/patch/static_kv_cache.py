...
def enable_duo_attention_static_kv_cache_for_qwen2(model: Qwen2ForCausalLM):
    model.model._prepare_decoder_attention_mask = lambda *args, **kwargs: None

    model.model.forward = types.MethodType(
        duo_attn_static_kv_cache_qwen2_model_forward, model.model
    )
    for idx in range(len(model.model.layers)):
        model.model.layers[idx].forward = types.MethodType(
            duo_attn_static_kv_cache_qwen2_decoder_layer_forward,
            model.model.layers[idx],
        )
    model.forward = types.MethodType(
        duo_attn_static_kv_cache_qwen2_for_causal_lm_forward, model
    )
