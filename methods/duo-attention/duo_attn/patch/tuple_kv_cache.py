...
def enable_tuple_kv_cache(model):
    if isinstance(model, LlamaForCausalLM):
        ...
        enable_tuple_kv_cache_for_llama(model)
    elif getattr(getattr(model, "config", None), "model_type", "") in ["chatglm", "glm"]:
        print("Skipping tuple KV cache patch for ChatGLM/GLM")
    else:
        raise ValueError(f"Model not supported: {type(model).__name__}")
