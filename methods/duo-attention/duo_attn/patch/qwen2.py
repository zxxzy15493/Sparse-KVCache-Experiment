...
try:
    from .static_kv_cache import enable_duo_attention_static_kv_cache_for_qwen2
except ImportError:
    from .static_kv_cache import enable_duo_attention_static_kv_cache_for_llama as enable_duo_attention_static_kv_cache_for_qwen2
from .tuple_kv_cache import enable_tuple_kv_cache_for_llama as enable_tuple_kv_cache_for_qwen2
            ...
            def make_patched_rope(original_func):
                def patched_rope_forward(x, seq_len=None):
                    ...
                    if seq_len is not None and isinstance(seq_len, torch.Tensor):
                        if seq_len.dim() >= 1:
                            actual_seq_len = seq_len.shape[-1]
                        else:
                            actual_seq_len = seq_len.item()
                        actual_seq_len = int(actual_seq_len)
                    else:
                        actual_seq_len = seq_len

                    return original_func(x, actual_seq_len)
...
#                 else:
#                     actual_seq_len = seq_len
#                 return original_rope_forward(x, actual_seq_len)

#             module.rotary_emb.forward = patched_rope_forward
