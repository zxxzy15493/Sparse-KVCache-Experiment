...
# def calculate_duo_attn_recall(gt_weights, full_attn_head_mask, sink_size, recent_size, num_kv_groups):
#     """
#     Calculate real recall for DuoAttention.
    
#     gt_weights: [bsz, num_heads, seq_len]
#     full_attn_head_mask: [num_kv_heads] (True if full, False if streaming)
#     sink_size: int
#     recent_size: int
#     num_kv_groups: int (num_heads // num_kv_heads)
#     """
#     if len(gt_weights.shape) == 4:
#         bsz, num_heads, q_len, seq_len = gt_weights.shape
#     else:
#         bsz, num_heads, seq_len = gt_weights.shape
    
#     num_kv_heads = full_attn_head_mask.shape[0]
    
#     # 1. Calculate total budget (number of tokens kept across all heads)
#     num_full_kv_heads = full_attn_head_mask.sum().item()
#     num_streaming_kv_heads = num_kv_heads - num_full_kv_heads
    
#     # Total tokens kept = (full heads * all tokens) + (streaming heads * window tokens)
#     window_tokens = sink_size + recent_size
#     if window_tokens > seq_len:
#         window_tokens = seq_len
        
#     k_total_heads = int((num_full_kv_heads * num_kv_groups * seq_len) +
#                         (num_streaming_kv_heads * num_kv_groups * window_tokens))
    
#     # 2. Get Gold Standard indices (Top-K globally)
#     # Decode Recall GT: Last Query Token
#     if len(gt_weights.shape) == 4: # [bsz, num_heads, q_len, kv_len]
#         full_attn_weights = gt_weights
        
#         # Scheme A: Decode Recall GT
#         attn_weights_decode_gt = full_attn_weights[:, :, -1, :] # [bsz, num_heads, full_seq_len]
        
#         # Scheme B: Prefill Recall GT
#         counts = torch.arange(seq_len, 0, -1, device=full_attn_weights.device, dtype=full_attn_weights.dtype)
#         attn_weights_prefill_gt = full_attn_weights.sum(dim=-2) / counts # [bsz, heads, seq_len]
        
#     elif len(gt_weights.shape) == 3: # Already pre-aggregated (Prefill only)
#         # This branch is for legacy support or when only prefill mean is passed
#         attn_weights_decode_gt = gt_weights # Assume it's the same for now or handle appropriately
#         attn_weights_prefill_gt = gt_weights
    
#     # Process both Decode and Prefill
#     results = {}
    
#     # Helper for calculation
#     def calc_single_recall(weight_tensor):
#         flat_weights = weight_tensor.reshape(bsz, -1)
#         batch_recalls = []
#         batch_attn_ratios = []
#         for b in range(bsz):
#             gt_flat_indices = flat_weights[b].topk(k_total_heads, dim=-1).indices
#             set_gt = set(gt_flat_indices.tolist())
            
#             # 3. Identify tokens selected by DuoAttention
#             selected_indices = []
#             for kv_h in range(num_kv_heads):
#                 is_full = full_attn_head_mask[kv_h]
#                 for h_offset in range(num_kv_groups):
#                     h = kv_h * num_kv_groups + h_offset
#                     head_offset = h * seq_len
                    
#                     if is_full:
#                         selected_indices.extend(range(head_offset, head_offset + seq_len))
#                     else:
#                         selected_indices.extend(range(head_offset, head_offset + sink_size)) 
#                         recent_start = max(sink_size, seq_len - recent_size)
#                         selected_indices.extend(range(head_offset + recent_start, head_offset + seq_len))
            
#             set_method = set(selected_indices)
#             overlap = len(set_gt.intersection(set_method))
            
#             recall = overlap / k_total_heads if k_total_heads > 0 else 1.0
#             if np.isnan(recall): recall = 1.0
#             batch_recalls.append(recall)
            
#             # Calculate Attention Ratio
#             method_selected_flat_tensor = torch.tensor(list(set_method), device=weight_tensor.device)
#             # Normalize by num_heads to get average ratio per head (0-1 range)
#             # Normalize by full sequence length if prefill (already handled in weight calculation usually, but for ratio sum check)
#             # Here we sum weights of selected tokens. Since weights are softmaxed per head, sum across heads should be num_heads.
#             # So we divide by num_heads to get 0-100%
#             attn_ratio = flat_weights[b, method_selected_flat_tensor].sum().item() / num_heads
            
#             # If prefill weights were not normalized by seq_len during creation (summed), we might need adjustment.
#             # But standard prefill_gt is (sum / counts), so it's a valid probability distribution per head.
#             # However, ensure weights sum to 1 per head.
            
#             batch_attn_ratios.append(attn_ratio)
            
#         return batch_recalls, batch_attn_ratios

#     decode_recalls, decode_attn_ratios = calc_single_recall(attn_weights_decode_gt)
#     prefill_recalls, prefill_attn_ratios = calc_single_recall(attn_weights_prefill_gt)
    
#     return decode_recalls, prefill_recalls, decode_attn_ratios, prefill_attn_ratios
