...
class ClusterKVModel:
    """ClusterKV model wrapper for HuggingFace models with ClusterKV attention acceleration."""
    def __init__(self, name_or_path: str, model_name: str,
                 token_budget: int = 1024, nlist: int = 400, sink: int = 16,
                 head_sel: str = "truc", balance: bool = False, fit_iter: int = 20,
                 dist_t: str = "cosine", gqa_policy: str = None,
        ...
        # Load model and tokenizer, matching mypred.py load_model_and_tokenizer style
        if "qwen" in model_name:
            # Qwen model loading style from mypred.py
            self.tokenizer = AutoTokenizer.from_pretrained(name_or_path, trust_remote_code=True)
            self.model = AutoModelForCausalLM.from_pretrained(
                name_or_path, trust_remote_code=True, torch_dtype=torch.bfloat16,
                device_map="auto", low_cpu_mem_usage=True,
                attn_implementation="flash_attention_2", use_cache=True
            )
        elif "llama" in model_name:
            # Llama model loading style from mypred.py
            self.tokenizer = AutoTokenizer.from_pretrained(name_or_path)
            self.model = LlamaForCausalLM.from_pretrained(
                name_or_path, torch_dtype=torch.bfloat16, device_map="auto", low_cpu_mem_usage=True,
                attn_implementation="flash_attention_2", use_cache=True
            )
        else:
            raise ValueError(f"Unsupported model_name: {model_name}. Only 'qwen' and 'llama' are supported.")
        self.model.eval()

        # Import and enable ClusterKV
        import sys
        import os
        # Create a fake package structure so relative imports in cluster_attention work
        accuracy_path = os.path.join(os.path.dirname(__file__), '../../accuracy')
        if accuracy_path not in sys.path:
            sys.path.insert(0, accuracy_path)
        # Make sure accuracy/ is treated as a package by adding its parent
        accuracy_parent = os.path.dirname(accuracy_path)
        if accuracy_parent not in sys.path:
            sys.path.insert(0, accuracy_parent)
        from accuracy.cluster_attention import cluster_reset
        from accuracy.patch import enable_attention_eval

        # Reset and enable ClusterKV attention
        args = types.SimpleNamespace(
            ...
            fit_iter=fit_iter,
            dist_t=dist_t,
            gqa_policy=gqa_policy,
            quest=False,
            cluster=True,
            cache_steps=0,
            topk_stat=False
        )
        enable_attention_eval(self.model_name, self.model, args)
    ...
    def process_batch(self, prompts: List[str], **kwargs) -> List[dict]:
        ...
        # Reset cluster cache for each request
        cluster_reset(self.model)
