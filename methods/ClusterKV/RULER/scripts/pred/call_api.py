...
parser.add_argument("--model_name", type=str, default=None,
                    help='model name for ClusterKV (e.g., llama3.1-8b-chat-32k, qwen2.5-7b-chat-32k)')

# ClusterKV options
parser.add_argument("--cluster", action="store_true", help="Enable ClusterKV attention")
parser.add_argument("--token_budget", type=int, default=1024, help="Token budget for ClusterKV")
parser.add_argument("--nlist", type=int, default=400, help="Number of clusters for KMeans")
parser.add_argument("--sink", type=int, default=16, help="Number of sink tokens to keep")
parser.add_argument("--head_sel", type=str, choices=["truc", "pad"], default="truc",
                    help="truncate or pad for different heads")
...
parser.add_argument("--dist_t", type=str, default="cosine",
                    choices=["cosine", "inner_product", "l2", "l1", "euclidean", "chebyshev", "canberra"],
                    help="Distance type for clustering")
...
if args.server_type == 'hf' or args.server_type == 'gemini' or args.server_type == 'clusterkv':
    args.threads = 1

save_dir_parts = str(args.save_dir).split('/')
max_length = 4096
if len(save_dir_parts) >= 2:
    max_length = int(save_dir_parts[-2])  # MAX_SEQ_LENGTH is the parent dir of 'pred'

args.nlist = max(args.nlist, max_length // 80)
print(args.nlist)
...
def get_llm(tokens_to_generate):
    ...
    elif args.server_type == 'clusterkv':
        from model_wrappers import ClusterKVModel
        llm = ClusterKVModel(
            name_or_path=args.model_name_or_path,
