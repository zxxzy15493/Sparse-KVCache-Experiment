...
for INPUT_LEN in 4096 8192 16384 32768 65536 131072; do
    if [[ "${RUN_LLAMA}" == "1" ]]; then
        run_one llama "${LLAMA_MODEL}" "${INPUT_LEN}" 2 1024
    fi
done
for INPUT_LEN in 262144; do
    if [[ "${RUN_LLAMA}" == "1" ]]; then
        run_one llama "${LLAMA_MODEL}" "${INPUT_LEN}" 2 1024
    fi
done
for INPUT_LEN in 4096 8192 16384 32768 65536 131072 ; do
    if [[ "${RUN_QWEN}" == "1" ]]; then
        run_one qwen "${QWEN_MODEL}" "${INPUT_LEN}" 2 1024
    fi
done
for INPUT_LEN in 262144 ; do
    if [[ "${RUN_QWEN}" == "1" ]]; then
