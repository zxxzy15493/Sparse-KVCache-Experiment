...
void KMBatchDecodeWithPagedKVCachePyTorchWrapper::BeginForward(torch::Tensor indptr,
...
void KMBatchDecodeWithPagedKVCachePyTorchWrapper::EndForward() {
	handler_.EndForward();
}

void
KMBatchDecodeWithPagedKVCachePyTorchWrapper::Forward(torch::Tensor q,
												   torch::Tensor o,
												   ...
												   float rope_scale,
												   float rope_theta) {
	if (paged_kv_indices.dim() == 1)
		ForwardImpl<true>(q, o, paged_kv_data, paged_kv_indices, paged_kv_indptr,
						paged_kv_last_page_len, paged_kv_last_page_idx,
						rope_scale, rope_theta);
	else
		ForwardImpl<false>(q, o, paged_kv_data, paged_kv_indices, paged_kv_indptr,
						paged_kv_last_page_len, paged_kv_last_page_idx,
						rope_scale, rope_theta);
}

template<bool full> inline void
KMBatchDecodeWithPagedKVCachePyTorchWrapper::ForwardImpl(torch::Tensor q,
												   torch::Tensor o,
												   torch::Tensor paged_kv_data,
												   torch::Tensor paged_kv_indices,
												   torch::Tensor paged_kv_indptr, // [1, Page_budget]
	...
	int64_t page_budget = 0;
	if constexpr (!full) {
		page_budget = paged_kv_indices.size(1);
	}
	...
	bool success = DISPATCH_PYTORCH_DTYPE_TO_CTYPE(q.scalar_type(), c_type, [&] {
		SWITCH_LAYOUT(kv_layout_, KV_LAYOUT, {
			paged_kv_t<PageStorage::kIndices, KV_LAYOUT, c_type, int32_t, full> paged_kv(
				num_kv_heads,
				page_size,
				head_dim,
				batch_size,
