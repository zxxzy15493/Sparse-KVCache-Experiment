...
#ifndef FLASHINFER_PAGE_CUH_KM_
#define FLASHINFER_PAGE_CUH_KM_
 ...
 */
template <PageStorage page_storage, QKVLayout layout, typename DType, typename IdType, bool full=false>
struct paged_kv_t {
	uint32_t num_heads;
	uint32_t page_size;
	uint32_t head_dim;
   ...
   */
	...
	get_k_ptr(IdType page_iter, uint32_t head_idx, uint32_t entry_idx, uint32_t feat_idx) const {
		if constexpr(page_storage == PageStorage::kIndices) {
			if constexpr (full)
				return data +
					get_k_elem_offset(page_iter, head_idx, entry_idx, feat_idx);
			else
				return data +
	...
	get_v_ptr(IdType page_iter, uint32_t head_idx, uint32_t entry_idx, uint32_t feat_idx) const {
		if constexpr(page_storage == PageStorage::kIndices) {
			if constexpr (full)
				return data +
					get_v_elem_offset(page_iter, head_idx, entry_idx, feat_idx);
			else
				return data +
	...
	// protective_get_v_ptr_heads is never used, both in original flashinfer and quest
	// Instead, they directly use protective_get_k_ptr and kv_offset_delta

	__device__ __forceinline__ DType* protective_get_k_ptr_page1(IdType page_iter,
																 uint32_t head_idx,
																 uint32_t feat_idx,
																 IdType last_indptr) const {
		if constexpr(page_storage == PageStorage::kIndices) {
			if(blockIdx.x == gridDim.x - 1) {
				// This is manully appended last page. Only one page here
				return data + get_k_elem_offset(last_page_idx, head_idx, 0, feat_idx);
			} else {
				// Note (Yilong):
				// indices: [num_kv_heads, page_budget - 1]. Manully exclude the last page for sake of Top-k.
				// Therefore, boundary check is last_indptr - 1 (since last_indptr is the last page index).
				if(page_iter < last_indptr - 1) {
					if constexpr (full)
						return data +
							get_k_elem_offset(page_iter, head_idx, 0, feat_idx);
					else
						return data +
							get_k_elem_offset(__ldg(indices + page_iter + head_idx * page_budget),
												head_idx,
		...
		for(int32_t entry_idx = start_entry_idx; entry_idx < end_entry_idx; ++entry_idx) {
			...
			local_k.cast_load(key +
							  ((append_indptr[batch_idx] + page_offset_bdx * paged_kv.page_size +
								entry_idx - append_start_seq) *	// why append_start_seq instead of start_entry_idx
								   num_heads +
							   head_idx) *
								  head_dim +
							  tx * vec_size);
