...
BatchDecodeWithPagedKVCacheKernel(DTypeIn* __restrict__ q,
								  paged_kv_t<page_storage, kv_layout, DTypeIn, IdType, full> paged_kv,
...
#pragma unroll
	for(uint32_t j = 0; j < tile_size_per_bdx; ++j) {
		k_ptrs_smem[((j * bdz + tz) * bdy + ty) * bdx + tx] = paged_kv.protective_get_k_ptr_page1(
			cur_page_indptr_begin + (((j * bdz + tz) * bdy + ty) * bdx + tx) / paged_kv.page_size,
			kv_head_idx,
			0,
			last_indptr);
...
#pragma unroll
			for(uint32_t j = 0; j < tile_size_per_bdx; ++j) {
				k_ptrs_smem[((j * bdz + tz) * bdy + ty) * bdx + tx] =
					paged_kv.protective_get_k_ptr_page1(
						cur_page_indptr_begin +
							((iter + num_stages_smem) * tile_size_per_bdx * bdy * bdz +
							 ((j * bdz + tz) * bdy + ty) * bdx + tx) /
								paged_kv.page_size,
...
__host__ __device__ __forceinline__ size_t get_elem_offset_sel_impl(size_t elem_idx, size_t head_idx,
...
struct tensor_info_sel_t {
  uint32_t qo_len;
  uint32_t kv_len;
  uint32_t num_kv_heads;
  __host__ __device__ __forceinline__ tensor_info_sel_t(uint32_t qo_len, uint32_t kv_len,
                                                    uint32_t num_kv_heads)
  ...
  __host__ __device__ __forceinline__ size_t get_qo_elem_offset(uint32_t qo_idx,
    ...
    return get_elem_offset_sel_impl<QKVLayout::kNHD, head_dim>(qo_idx, qo_head_idx, feat_idx, qo_len,
                                                           get_num_qo_heads());
  }
  ...
  __host__ __device__ __forceinline__ size_t get_kv_elem_offset(uint32_t kv_idx,
                                                                ...
                                                                uint32_t feat_idx) const {
    return get_elem_offset_sel_impl<kv_layout, head_dim>(kv_idx, kv_head_idx, feat_idx, kv_len,
                                                     num_kv_heads);
  }
...
__global__ void SingleDecodeWithKVCacheKernel(DTypeIn* __restrict__ q, DTypeIn* __restrict__ k,
                                              ...
                                              DTypeOut* __restrict__ tmp,
                                              tensor_info_sel_t<kv_layout, bdy, bdx * vec_size> info,
...
#pragma unroll 2
  for (uint32_t iter = 0; iter < ceil_div(kv_chunk_size, tile_size_per_bdx * bdy * bdz); ++iter) {
    ...
    compute_qk<rotary_mode, vec_size, bdx, bdy * tile_size_per_bdx>(
		...
		q_vec,
        freq, 
		iter * bdy * tile_size_per_bdx * bdz, 
		kv_chunk_size, 
		sm_scale,
        s, 
		st_local);
      ...
      {SWITCH_HEAD_DIM(
          ...
          {SWITCH_ROTARY_MODE(
              rotary_mode, ROTARY_MODE, {SWITCH_LAYOUT(kv_layout, KV_LAYOUT, {
                ...
                constexpr uint32_t bdz = num_threads / (bdx * bdy);
                tensor_info_sel_t<KV_LAYOUT, GROUP_SIZE, HEAD_DIM> info(1, seq_len, num_kv_heads);
                constexpr uint32_t tile_size_per_bdx =
