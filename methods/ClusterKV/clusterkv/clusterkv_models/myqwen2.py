# FlashInfer-free ClusterKV implementation for Qwen2.
#
# Goals
# -----
...
#   clusterkv/clusterkv_models/qwen2.py
...
""" Qwen2 model using ClusterKV native cache/kernels and flash-attn. """
...
class OriginalKVCacheAdapter:
    ...
    def append(self, layer_idx: int, new_k: torch.Tensor, new_v: torch.Tensor) -> None:
        # myqwen2 produces [Hk, q_len, D]; original append_kv expects [q_len, Hk, D].
        append_kv(
...
class MyQwen2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        assert bsz == 1, "MyQwen2DecoderLayer only supports batch size 1."
...
# ---------------------------------------------------------------------------
# Qwen2Model: owns the shared rotary embedding, the cache, and the layer loop
# ---------------------------------------------------------------------------
...
class MyQwen2Model(Qwen2PreTrainedModel):
    def __init__(self, config: Qwen2Config):
        ...
        self.layers = nn.ModuleList(
            [MyQwen2DecoderLayer(config, i) for i in range(self.num_layers)]
        )
...
# ---------------------------------------------------------------------------
# Qwen2ForCausalLM (entry point with the same external API as the original)
# ---------------------------------------------------------------------------
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def clusterkv_init(
        ...
        print(
            f"MyQwen2ForCausalLM: ClusterKVController cache allocated (max_seq_len={max_seq_len}, "
    ...
    def forward(
        ...
        # Qwen2ForCausalLM and matching what bench_textgen.py expects.
    ...
    def _reorder_cache(past_key_values, beam_idx):
        ...
        raise NotImplementedError(
            "Beam search is not supported by MyQwen2ForCausalLM."
        )
