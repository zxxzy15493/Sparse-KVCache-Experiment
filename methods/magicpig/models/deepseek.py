...
import flashinfer
from .attnserver_dist_prune import LSHSparseAttnServer
...
class LLMLayer:
	...
	def init_parameters(self, hf_layer: Qwen2DecoderLayer):
		...
		self.wq = self.wq.split((self.num_heads * self.head_dim) // self.world_size, dim=0)[self.rank]

		self.bq = hf_layer.self_attn.q_proj.bias.detach()
		self.bq = self.bq.split((self.num_heads * self.head_dim) // self.world_size, dim=0)[self.rank]

		self.wk = hf_layer.self_attn.k_proj.weight.detach()
		self.wk = self.wk.split(self.key_value_slicing, dim=0)[self.rank]

		self.bk = hf_layer.self_attn.k_proj.bias.detach()
		self.bk = self.bk.split(self.key_value_slicing, dim=0)[self.rank]

		self.wv = hf_layer.self_attn.v_proj.weight.detach()
		self.wv = self.wv.split(self.key_value_slicing, dim=0)[self.rank]

		self.bv = hf_layer.self_attn.v_proj.bias.detach()
		self.bv = self.bv.split(self.key_value_slicing, dim=0)[self.rank]
		...
		self.gate_proj = self.gate_proj.split(self.mlp_slice, dim=0)[self.rank]

		self.up_proj = hf_layer.mlp.up_proj.weight.detach()
		self.up_proj = self.up_proj.split(self.mlp_slice, dim=0)[self.rank]
...
class DeepSeekModel:
	...
	def generate(
		...
		torch.cuda.synchronize()
		self.prefill_latency = time.perf_counter() - prefill_start
		...
		for k in range(max_tokens):
			...
			if eos_token_ids and token_id in eos_token_ids and self.fixed_output_length == 0:
				print(f"Early stop at step {k}, generated token id: {token_id}")
				break
