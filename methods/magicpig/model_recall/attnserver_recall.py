    ...
    def decode(
        ...
        if layer_idx in self.dense_layers:
            ...
            if self.RECALL:
                ...
                attention_scores = torch.matmul(query_states, past_key_states.transpose(-1, -2)) / self.head_dim**0.5
                # attn_weights shape --> (batch_size, num_attention_heads, 1, seq_len)
                attn_weights = torch.softmax(attention_scores, dim=-1)

                # headrecall
                topBudget_weight, topBudget_indices = attn_weights.topk(self.nnz.max(), dim=-1)
                topBudget_indices = topBudget_indices.reshape(-1, self.nnz.max())
                top100_weight, top100_indices = attn_weights.topk(100, dim=-1)
                top100_indices = top100_indices.reshape(-1, 100)
                layer_recall_budget = {
                    "layer_id": layer_idx,
                    "recall": 0,
                    "head_recall": [],
                ...
                for query_head in range(self.num_attention_heads):
                    retrieved_indices = self.results_lsh_cpu[:, :self.nnz[query_head]]
                    topBudget_set = set(topBudget_indices[query_head].tolist())
                    top100_set = set(top100_indices[query_head].tolist())
                    # retrieved token id,  num_sink_tokens
                    retrieved_set = [idx + self.num_sink_tokens for idx in retrieved_indices[query_head].tolist()]
                    # print(colored(f"layer_id:{layer_idx} {query_head} retrieved_set before adding sink and local tokens: {len(retrieved_set)}", "yellow"))
                    retrieved_set += (list(range(self.num_sink_tokens)) + list(range(self.prefill_len-self.num_local_tokens, attn_weights.shape[-1])))
                    
                    # set，
                    if len(retrieved_set) != len(set(retrieved_set)):
                        pass
                    retrieved_set = set(retrieved_set)
                    intersection = topBudget_set.intersection(retrieved_set)
                    top100_intersection = top100_set.intersection(retrieved_set)
                    total_recall = len(intersection)
                    total_count = len(topBudget_set) 

                    recall = total_recall / total_count if total_count > 0 else 0
                    top100_recall = len(top100_intersection) / 100
                    retrieved_set = list(retrieved_set)
                    LSH_attn_weight = attn_weights[0, query_head, 0, retrieved_set].sum().item()
                    
                    topk_attn_weight = attn_weights[0, query_head, 0, topBudget_indices[query_head]].sum().item()
                    
                    # print(colored(f"layer_id:{layer_idx} {query_head} LSH Attention Recall: {recall:.4f}," + 
                    #               f" total recall is {total_recall}," + 
                    #                   f" {LSH_attn_weight:.4f}, TopK{topk_attn_weight:.4f} or {topk_attn_weight==LSH_attn_weight}" +
                    #                     f" {attn_weights.shape[-1]} k {len(topBudget_indices[query_head])} and retrieved size is {len(retrieved_set)}" + 
                    #                     f" self.nnz.max() is {self.nnz.max()}", "magenta"))

                    layer_recall_budget["head_recall"].append(recall)
                    layer_attn_weight_budget["head_attn_weight"].append(LSH_attn_weight)
                    layer_recall_100["head_recall"].append(top100_recall)
                    
                layer_recall_100["recall"] = sum(layer_recall_100["head_recall"]) / self.num_attention_heads
                layer_recall_budget["recall"] = sum(layer_recall_budget["head_recall"]) / self.num_attention_heads
                layer_attn_weight_budget["attn_weight"] = sum(layer_attn_weight_budget["head_attn_weight"]) / self.num_attention_heads
                ...
                # recalldecodeStepData，，
                self.decodeStepData_topBudget["layer_recall_list"].append(layer_recall_budget["recall"])
                self.decodeStepData_topBudget["layer_attn_weight_list"].append(layer_attn_weight_budget["attn_weight"])
                self.decodeStepData_top100["layer_recall_list"].append(layer_recall_100["recall"])
                ...
                # print(colored(f"layer_id:{layer_idx}  LSH Attention Recall: {layer_recall_budget['recall']:.4f}", "magenta"))
