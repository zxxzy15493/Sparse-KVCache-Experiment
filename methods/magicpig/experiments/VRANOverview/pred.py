...
def get_pred(llm, data, max_new_tokens, pred_dir, args):
    ...
    print("="*80)
    print("="*80)
    peak_alloc = torch.cuda.max_memory_allocated() / (1024 ** 2)  # Convert to MB
    torch.cuda.empty_cache()
    out_path = Path(pred_dir) / f"VRAMOverview_{args.input_max_token}_{args.K}_{args.L}.jsonl"
    pred = generated_text
...
def main(args):
    ...
    max_length = args.input_max_token + max_new_tokens + 64

    ### ：
    ###  max_length: kvcache， input_max_token + max_new_tokens
    ###  generation_buffer: kvcache， max_new_tokens
    llm = load_model(model_path, K, L, 1, max_length, max_new_tokens, DEVICE, dtype, args)
