...
PRED_DIR="./results/efficencyBudget/${MODEL}/${INPUT_MAX_TOKEN}_${fixed_output_length}/${except_budget}"
LOG_DIR="./log/efficencyBudget/${MODEL}/${INPUT_MAX_TOKEN}_${fixed_output_length}/"
mkdir -p ${PRED_DIR}
mkdir -p ${LOG_DIR}
...
CUDA_VISIBLE_DEVICES=0 \
        OMP_NUM_THREADS=96 \
        torchrun --nproc_per_node=1 ./timeBreakdown.py \
        --data_dir ${DATA_DIR} \
        --save_dir ${PRED_DIR} \
        --model_name ${MODEL} \
        --K ${K} \
        ...
        > ${LOG_DIR}/${except_budget}_${KLS}.log 2>&1
