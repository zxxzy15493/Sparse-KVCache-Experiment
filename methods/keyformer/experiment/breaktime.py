...
import keyformer_kv.keyformer_time as K

def parse_args(args=None):
    p = argparse.ArgumentParser()
    p.add_argument('--model', type=str, required=True)
    p.add_argument('--key_size', type=int, default=992)
    p.add_argument('--recent_size', type=int, default=32)
    p.add_argument('--tau_init', type=float, default=1.0)
    p.add_argument('--tau_delta', type=float, default=0.01)
    p.add_argument('--e', action='store_true')
    p.add_argument('--dataset', type=str, default="../../benchmarks/myinput.txt")
...
def build_output_path(model_name, dataset_name, dataset_path, max_length, key_size=None, recent=None):
    base = "./time_profile_results"
    budget_dir = f"budget{key_size + recent}" if (key_size and recent) else "budget_full"
...
def collect_global_times(max_gen):
    K.global_prefill_start_event.synchronize()
    K.global_prefill_end_event.synchronize()
    prefill_total = K.global_prefill_start_event.elapsed_time(K.global_prefill_end_event) / 1000.0
    K.global_decode_start_event.synchronize()
    K.global_decode_end_event.synchronize()
    decode_total = K.global_decode_start_event.elapsed_time(K.global_decode_end_event) / 1000.0
...
if __name__ == '__main__':
    ...
    K.enable_keyformer_time(model, key_size=args.key_size, recent_size=args.recent_size,
                             tau_init=args.tau_init, tau_delta=args.tau_delta)
    ...
    for max_length in [4096, 65536]:
        ...
        for run_idx in range(args.num_runs):
            ...
            K.global_decode_end_event.record()
            ...
            K.breaktime_registry.append(entry)
