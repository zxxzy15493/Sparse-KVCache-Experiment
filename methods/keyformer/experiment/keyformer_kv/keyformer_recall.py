...
def common_diagnostic_and_evict(self, query_states, key_states, value_states, past_key_value, bsz, q_len, sin, cos):
    ...
    if kv_seq_len <= self.kv_cache.cache_size:
        ...
        history_boundary = kv_seq_len - self.kv_cache.recent_size
        keep_recent_idx = torch.arange(history_boundary, kv_seq_len, device=device).view(1, 1, -1).expand(bsz, num_heads, -1)
        
        history_scores = self.kv_cache.key_score[:, :, : history_boundary]
        ...
        self.kv_cache.select_idx = keep_idx
        
        global_indices_gathered = torch.gather(self.kv_cache.absolute_indices, -1, keep_idx)
        current_select_global = global_indices_gathered[0]
    ...
    if kv_seq_len > self.kv_cache.cache_size:
        self.kv_cache.absolute_indices = global_indices_gathered
...
def enable_keyformer_recall(model, args):
    try:
    ...
    total_budget = args.recent_size + args.key_size

    patched_layers = 0
    for name, module in model.named_modules():
        if llama_classes and isinstance(module, llama_classes):
            module.kv_cache = KeyformerKVCache(total_budget=total_budget, recent_size=args.recent_size)
    ...
    print(f"Keyformer Global Diagnostic Patch Enabled. Patched {patched_layers} attention layers (Budget={total_budget}).")