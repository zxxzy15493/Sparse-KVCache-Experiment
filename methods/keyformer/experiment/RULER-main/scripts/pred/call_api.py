...
parser.add_argument("--enable_keyformer", action="store_true")
...
def get_llm(tokens_to_generate):
    if args.server_type == 'trtllm':
        ...
        llm = HuggingFaceModel(
            ...
            enable_keyformer=args.enable_keyformer,
...
def main():
    ...
    if args.server_type == 'hf' and args.check_recall and args.enable_keyformer:
        try:
            from keyformer_kv.keyformer_recall import init_new_sample_registry, stats_registry
            import csv
            import numpy as np
            ...
            print(f"\nRULER Task '{args.task}' (Keyformer) completed. Consolidating metrics...")
            ...
            print(f"CSV metric matrix generated! Total lines logged: {row_count}.")
            np.savez_compressed(stats_path_npz, data=stats_registry)
            print(f"Binary archive backup generated at: {stats_path_npz}")
            ...
            if total_element_count > 0:
                ...
                print("\n" + "="*60)
                print(f"KEYFORMER KV CACHE DIAGNOSTIC FINAL REPORT ({args.task})  📊")
                print(f"Raw report exported to: {stats_path_csv}\n")
            else:
                print("\nNo metrics collected during inference.\n")

        except Exception as e:
            print(f"Matrix compilation failed: {e}")
            traceback.print_exc()
