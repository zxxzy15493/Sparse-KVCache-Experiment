...
class HuggingFaceModel:
    def __init__(self, name_or_path: str, **generation_kwargs) -> None:
        ...
        if self.enable_keyformer:
            curr_model.config.key_size = self.key_size
            curr_model.config.recent_size = self.recent_size
            curr_model.config.tau_init = self.tau_init
            curr_model.config.tau_delta = self.tau_delta
            from keyformer_kv.enable_keyformer import enable_keyformer
            
            class KeyformerArgs:
                def __init__(self, key_size, recent_size, tau_init, tau_delta, check_recall):
    ...
    def reset_sample_cache_state(self):
        if not getattr(self, "enable_keyformer", False):
            return
        curr_model = self.pipeline.model if self.pipeline else self.model
        model_type = getattr(getattr(curr_model, "config", None), "model_type", "")
        if "llama" in model_type:
            ...
            try:
                import keyformer_kv.modify_qwen as modify_qwen
