...
def scorer(dataset, predictions, answers, all_classes):
    if dataset not in Ruler:
        ...
        if len(answers) > 0 and answers[0][0] is not None:
            return dataset2metric[dataset](predictions, answers)
        else:
            return 0.0
...
if __name__ == '__main__':
    ...
    scores = dict()
    if args.e:
        path = f"output_e/{args.model}/"
    else:
        path = f"/keyformer/experiment/output/{args.model}/"
    ...
    if args.e:
        ...
        out_path = f"/keyformer/experiment/output/{args.model}/result.json"
