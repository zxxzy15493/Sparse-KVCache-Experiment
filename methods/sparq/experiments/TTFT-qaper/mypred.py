...
def parse_args():
  ...
  parser.add_argument("--output_dir", type=Path, default="pred", help="Output directory for predictions")
  parser.add_argument("--model_name", type=str, default="", help="Model name for chat template and output directory")
  parser.add_argument("--name", type=str, required=True, help="Sparsity method name for ANN selection")
  ...
  parser.add_argument("--score", type=str, required=True, help="Scoring method")
  ...
  parser.add_argument("--type", type=str, required=True, help="Metric type: TTFT, THROUGHPUT, LATENCY, recall, or topkrate")
  ...
  parser.add_argument("--max_new_tokens", type=int, default=128, help="Max new tokens to generate (TTFT mode forces this to 1)")
...
def load_prompt_from_file(file_path: str) -> str:
  """Read entire file content as prompt."""
...
def latency(
  ...
  for _ in range(4):
    for json_obj in tqdm(data, desc=f"Task={task_name}"):
      ...
      latency_val = sum(decode_latency)
...
def throughput(
  ...
  for json_obj in tqdm(data, desc=f"Task={task_name}"):
    ...
    throughput_val = new_token_num / (ms / 1000.0)
