...
def get_llm(tokens_to_generate):
  if args.server_type == 'trtllm':
    ...
    from sparq_model_wrappers import HuggingFaceModel
    sparsity_cfg = {
    "name": args.name,
    "k": args.k,
    ...
    "type": "recall",
    "recall_save_path": args.recall_save_path + "/" + args.task,
    }
...
def main():
  ...
  def get_output(idx_list, index_list, input_list, outputs_list, others_list, truncation_list, length_list):
    ...
    while True:
      try:
        pred_list = llm.process_batch(prompts=input_list, sample_ids=index_list)
        break
