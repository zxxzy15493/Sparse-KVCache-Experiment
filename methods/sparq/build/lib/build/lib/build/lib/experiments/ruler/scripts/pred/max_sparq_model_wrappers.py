...
class HuggingFaceModel:
    ...
    def __init__(self, name_or_path: str, sparsity:Optional[Dict[str, Any]] = None, **generation_kwargs) -> None:
        ...
        # print("attn impl =", getattr(self.model.config, "_attn_implementation", None))
        # print("attn class =", type(self.model.model.layers[0].self_attn))

        sparq = Sparsity(
                name=sparsity.get("name", "ann"),
                k=int(sparsity.get("k", 512)),
                local_k=int(sparsity.get("local_k", 256)),
                rank=int(sparsity.get("rank", 64)),
                score=sparsity.get("score", "sparse_q"),
                reallocate_to_mean_value=bool(sparsity.get("reallocate_to_mean_value", True)),
            )
    ...
    def process_batch(self, prompts: List[str], **kwargs) -> List[dict]:
        if self.pipeline is None:
            ...
            print(" token :", inputs["attention_mask"].sum(dim=1).tolist())
