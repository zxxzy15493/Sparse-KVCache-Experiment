...
from . import eval_adapter, utility####dddd
from .methods import ann_attention_copy, eviction_attention, sparse_attention
...
MODELS = (GPTNeoXForCausalLM, LlamaForCausalLM, MistralForCausalLM, GemmaForCausalLM,Qwen2ForCausalLM)
...
def _evaluate(
    ...
    if task.name in ["triviaqa", "squad", "squad_train"]:
    ...
    elif task.name == "longbench_qasper":

        qasper_path = "benchmarks/Longbench_recall/qasper.jsonl"


        data_list = []
        with open(qasper_path, encoding="utf-8") as f:
            for line in f:
        ...
        n = min(task.samples, len(data_list))
        ...
        examples = [
            qa.add_few_shot_prompt(
                ...
                k=task.shots,
                prompt_template=qa.get_default_prompt_template(
                adapter.model.config._name_or_path,
                task.shots,
            ),
        )
            for i in range(n)
