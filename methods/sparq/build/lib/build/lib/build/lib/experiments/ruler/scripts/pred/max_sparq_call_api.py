...
def get_llm(tokens_to_generate):
    if args.server_type == 'trtllm':
        ...
        from max_sparq_model_wrappers import HuggingFaceModel
        sparsity_cfg = {
        "name": args.name,
        "k": args.k,
...
def main():
    ...
    if args.chunk_amount > 1:
        pred_file = args.save_dir / f'{args.task}-{suffix}-{args.chunk_idx}.jsonl'
    else:
        pred_file = args.save_dir / f'{args.task}-{suffix}-.jsonl'
