...
class AnnAttention(nn.Module):
    ...
    def caculate_recall(
        ...
        scores_full = (query.div(query.shape[-1] ** 0.5) @ key.transpose(-1, -2)).add_(
            logmask
        )
        ...
        topk_score = scores_full.sum(dim=2, keepdim=True)  # (B, n_kv, 1, 1, seq)

        cur_seq = int(key.shape[-2])

        if self._first_decode_seq is None:
            self._first_decode_seq = cur_seq

        token_no = cur_seq - self._first_decode_seq + 1
        ...
        # full_indices: (B, n_kv, 1, 1, K) -> (B, n_kv, K)
        full_token_ids = full_indices.squeeze(2).squeeze(2)  # 0-based position

        full_token_ids_list = full_token_ids.detach().cpu().tolist()

        sparq_indices = sparqIndices

        K = full_indices.shape[-1]

        real = full_indices.flatten(0, 1)    # (B*n_kv, 1, 1, k)
        sparq = sparq_indices.flatten(0, 1)  # (B*n_kv, 1, 1, k)


        ground_truth_idx_cnt = k_eff * full_indices.shape[0] * full_indices.shape[1]
        hit_idx_cnt = 0
        ...
        for i in range(sparq.shape[0]):
            ...
            hit = (d.unsqueeze(-1) == r.unsqueeze(-2)).any(dim=-1)  # (k,)
            hit_idx_cnt += hit.int().sum()

        result = hit_idx_cnt.float() / float(ground_truth_idx_cnt)

        #self.recall_history.append(result.item())
        #np_hist = np.array(self.recall_history, dtype=np.float32)
        #mean, var = float(np.mean(np_hist)), float(np.var(np_hist))
        record = {
            "index":token_no,
        ...
        with open(self.settings.recall_save_path, "a", encoding="utf-8") as f:
    ...
    def caculate_topk_rate(
        ...
        # 2) full attention weights
        full_weights = torch.softmax(scores_full, dim=-1)  # float32
        ...
        record = {
            "token_index": token_no,
            "layer": self.layer_idx,
            "k": sparq_indices.shape[-1]+1,
    ...
    def forward(
        ...
        elif self.settings.type=="topkrate":
  ...
  # Note: expand indices as scatter does not broadcast (!)
        # Ensure weights dtype matches logmask dtype for scatter
        weights = weights.to(logmask.dtype)
...
class GPTNeoXAttentionWithANN(GPTNeoXAttention):  # type:ignore[misc]
    def __init__(self, config: GPTNeoXConfig, settings: Settings):
        #utility.check_transformers_version(type(self))        super().__init__(config)
...
class LlamaAttentionWithANN(llama_attention02.LlamaAttention):
    def __init__(
        ...
        #utility.check_transformers_version(type(self))        super().__init__(config, layer_idx)
        self.settings = settings
...
class Qwen2AttentionWithANN(qwen_attention.Qwen2Attention):
    def __init__(
        ...
        if n_kv_heads is None:
            n_kv_heads = getattr(self, "num_heads")
        ...
        self.ann = AnnAttention(settings, n_kv_heads, head_dim)
        self.ann.layer_idx = layer_idx
...
def convert(model: Model, settings: Settings) -> Model:
    ...
    def _replace(m: nn.Module) -> Optional[nn.Module]:
        ...
        if isinstance(m, QWEN2_ATTENTION_TYPES):
            return Qwen2AttentionWithANN(model.config, m.layer_idx, settings)
        if isinstance(m, GPTNeoXAttention):
            return GPTNeoXAttentionWithANN(model.config, settings)
        if isinstance(m, LLAMA_ATTENTION_TYPES):
