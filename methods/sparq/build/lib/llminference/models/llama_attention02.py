...
# Modified by Graphcore to use FlashAttention for memory efficiency
#
...
class LlamaAttention(modeling_llama.LlamaAttention):
    ...
    def _attn(
        ...
        if FLASH_ATTN_AVAILABLE:
            ...
            # Adjust shapes for FlashAttention: [batch, seq, heads, dim]
            ...
            if attention_mask is not None:
                # attention_mask: [b, 1, q_len, seq_len] -> [b, q_len, seq_len]
                attn_mask = attention_mask.squeeze(1)
                # Convert to boolean mask if needed (assuming it's additive mask with -inf)
                # FlashAttention expects attn_mask as float or None
            else:
                attn_mask = None

            # Call FlashAttention
            # Note: Use causal=True for causal masking
            ...
            # Adjust back to original shape: [b, h, s, d]
    ...
    def forward(
        ...
        **kwargs,
        ...
        # MODIFIED - generate attention_mask if None (for compatibility with flash_attention_2)
        bsz, q_len, _ = hidden_states.size()
        if attention_mask is None:
            # Generate causal mask: upper triangle is -inf, lower triangle is 0
            ...
            # Will be expanded later to match seq_len

        # MODIFIED - removed assert since we generate mask if None
        ...
        if attention_mask is None:
            # Generate causal mask for the current sequence
