...
import streaming_llm.streaming_time as S

def parse_args(args=None):
    p = argparse.ArgumentParser()
    p.add_argument('--model', type=str, required=True)
    p.add_argument('--start_size', type=int, default=16)
    p.add_argument('--recent_size', type=int, default=1008)
...
def build_output_path(model_name, dataset_name, dataset_path, max_length, start=None, recent=None):
    base = "./time_profile_results"
    budget_dir = f"budget{start + recent}" if (start and recent) else "budget_full"
...
def collect_global_times(max_gen):
    S.global_prefill_start_event.synchronize()
    S.global_prefill_end_event.synchronize()
    prefill_total = S.global_prefill_start_event.elapsed_time(S.global_prefill_end_event) / 1000.0
    S.global_decode_start_event.synchronize()
    S.global_decode_end_event.synchronize()
    decode_total = S.global_decode_start_event.elapsed_time(S.global_decode_end_event) / 1000.0
...
if __name__ == '__main__':
    ...
    S.enable_streaming_llm_time(model, start_size=args.start_size, recent_size=args.recent_size)
    ...
    for max_length in [4096, 65536]:
        ...
        for run_idx in range(args.num_runs):
            ...
            S.global_decode_end_event.record()
            ...
            S.breaktime_registry.append(entry)
