...
class HuggingFaceModel:
    def __init__(self, name_or_path: str, **generation_kwargs) -> None:
        ...
        self.start_size = generation_kwargs.pop('start_size', 992)
        ...
        if self.enable_streaming:
            ...
            from streaming_llm.streaming_recall import enable_streaming_llm_recall
            import streaming_llm.streaming_recall as sr
            class SubArgs:
                pass
            s_args = SubArgs()
            s_args.start_size = self.start_size
            s_args.recent_size = self.recent_size
            s_args.check_recall = self.check_recall

            enable_streaming_llm_recall(self.model, s_args)
            print(f"RULER StreamingLLM injected successfully. StartSize:{self.start_size}, RecentSize:{self.recent_size}")
    ...
    def reset_sample_cache_state(self):
        if self.check_recall and self.enable_streaming:
            from streaming_llm.streaming_recall import init_new_sample_registry
            import streaming_llm.streaming_recall as sr
            init_new_sample_registry()
            sr._SHARED_DECODING_STEP = 0
