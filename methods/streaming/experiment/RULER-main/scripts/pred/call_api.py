...
parser.add_argument("--enable_streaming", action="store_true", help="Enable StreamingLLM compression")
parser.add_argument("--start_size", type=int, default=992, help="Attention Sinks size (initial tokens preserved)")
parser.add_argument("--recent_size", type=int, default=32, help="Rolling local window size")
...
def main():
    ...
    if args.server_type == 'hf' and args.enable_streaming and args.check_recall:
        try:
            from streaming_llm.streaming_recall import init_new_sample_registry, stats_registry
            import csv
            import numpy as np
            ...
            print(f"\nRULER Task '{args.task}' (StreamingLLM) done. Processing metrics...")
            row_count = 0
