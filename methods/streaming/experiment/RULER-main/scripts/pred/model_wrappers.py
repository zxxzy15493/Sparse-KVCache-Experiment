...
class HuggingFaceModel:
    def __init__(self, name_or_path: str, **generation_kwargs) -> None:
        ...
        self.enable_streaming = generation_kwargs.pop('enable_streaming', False)
        self.start_size = generation_kwargs.pop('start_size', 16)
        self.recent_size = generation_kwargs.pop('recent_size', 1008)
        ...
        if self.enable_streaming:
            ...
            curr_model.config.start_size = self.start_size
            curr_model.config.recent_size = self.recent_size
            curr_model.config.check_recall = self.check_recall
            
            class StreamingArgs:
                def __init__(self, s, r,recall):
                    self.start_size = s 
                    self.recent_size = r
    ...
    def reset_sample_cache_state(self):
        if not getattr(self, "enable_streaming", False):
            return
        curr_model = self.pipeline.model if self.pipeline else self.model
        try:
            import streaming_llm.pos_shift.modify_llama as modify_llama
            modify_llama.step_layer_recalls.clear()
            modify_llama.step_layer_recalls["_global_step"] = 0
        except:
            pass

        try:
            import streaming_llm.pos_shift.modify_qwen as modify_qwen
            modify_qwen.step_layer_recalls.clear()
            modify_qwen.step_layer_recalls["_global_step"] = 0
        except:
            pass

        target_names = {
            "SelfAttention", "LlamaAttention", "LlamaSdpaAttention",
            "LlamaFlashAttention2", "Qwen2Attention", "Qwen2SdpaAttention",
            "Qwen2FlashAttention2",
        }
        for _, module in curr_model.named_modules():
            if module.__class__.__name__ in target_names:
                if getattr(module, "check_recall", False):
                    module.absolute_full_k = None
