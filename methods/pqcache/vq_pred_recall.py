...
def parse_args(args=None):
    ...
    compressor_choices = ["h2o", "original", "no_drop_lb", "pq_search","sparq_f", "no_drop_lb_32", "topp", "no_drop_lb_topp", "no_drop_lb_topp32"]
    parser.add_argument('--model', type=str, default=None, choices=[
        ...
        "qwen-2.5-7b", "qwen-2.5-7b-1m", "llama-3.1-8b", "glm-9b", "chatglm"])
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Dataset names to evaluate. If not set, evaluate the default dataset list."
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        default="./recall_data",
        help="Directory containing local LongBench recall JSONL files."
    )
...
def get_config_str_list(args):
    ...
    elif args.compressor in ["off","vq_h2o","no_drop_lb","snapkv","h2o","no_drop_lb_32"]:
        config_str = [ 
            f"budget_{args.budget}",
            f"rec_{args.recent_size}",
            f"sink_{args.sink_size}",
    ...
    elif args.compressor in ["topp", "no_drop_lb_topp", "no_drop_lb_topp32"]:
        config_str = [
            f"fixthreshold_{args.fixthreshold}",
            f"mode_{args.compressor}",
            f"gqa_{args.gqa}",
        ]
    return config_str
...
if __name__ == '__main__':
    ...
    #                 "passage_count", "passage_retrieval_en"]
    # datasets = ["vcsum", "trec", "triviaqa", "samsum", "lsht", \
    #                 "passage_count", "passage_retrieval_en", "passage_retrieval_zh", "lcc", "repobench-p"]
    # datasets = ["qasper"]
    # datasets = ["lcc", "repobench-p"]
    # datasets = ["narrativeqa", "qasper", "gov_report", "trec", "lcc"]
    # datasets = ["narrativeqa", "qasper", \
    #         "2wikimqa", "musique", \
    #         "gov_report", "multi_news", \
    #         "triviaqa", "samsum", \
    #         "passage_count", "passage_retrieval_en", \
    #         "lcc", "repobench-p"]
    # datasets = ["qasper", "passage_retrieval_en"]
    # datasets = ["trec"]
    ...
    for dataset in datasets:
        ...
        if args.e:
            data = load_dataset('./recall_data', f"{dataset}_e", split='test')
            if not os.path.exists(f"pred_e/{model_name}"):
                os.makedirs(f"pred_e/{model_name}")
            out_path = f"pred_e/{model_name}/{dataset}.jsonl"
        else:
            data = load_dataset(
                'json', data_files=os.path.join(args.data_dir, dataset + '.jsonl'), split='train'
            )
