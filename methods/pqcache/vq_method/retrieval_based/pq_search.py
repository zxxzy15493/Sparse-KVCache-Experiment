...
from .global_timer import global_timer, stage_begin, stage_end

CHECK_RECALL = eval(os.environ.get("CHECK_RECALL", "0"))
SYNC_TEST_TIME = eval(os.environ.get("SYNC_TEST_TIME","0"))
LAYER_ATTENTION_TIMING = eval(os.environ.get("PQCACHE_LAYER_ATTENTION_TIMING", "0"))

pq_compute_time = 0


def _record_internal_stage_time():
    return SYNC_TEST_TIME and global_timer.can_record()
...
def initialize_objects(config, model):
    ...
    MAX_CPU_IN_USE=int(eval(os.environ.get("MAX_CPU_IN_USE", "48")))
    MAX_WORKER_CNT=64
    ...
    pp_size = len(visible_devices.split(",")) if visible_devices else 1
    ...
    for rank in range(pp_size):
        cache_manager = init_gpu_cache_manager(
                                        ...
                                        budget = getattr(config, 'budget', 1024),
...
class PqBasedSearchCompressor(RetrievalBasedCompressor):
    ...
    def __init__(self, compress_ratio, recent_ratio, n_subvec_per_head, n_subbits, gqa, sink_size = 16, fixbudget = True, budget = 1024, **kwargs):
        ...
        self.recent_size = kwargs.get("recent_size", 32)  # Get from kwargs, default 32
    ...
    def prefetch_codebook(self):
        with torch.cuda.stream(H2DStream):
            if _record_internal_stage_time():
                self.prefetch_event_start.record()
            self.gpu_centroids = self.centroids.to(self.device, non_blocking=True).to(torch.bfloat16)
    ...
    def predict_index_gpu(self, vec: torch.Tensor):
        ...
        distances = torch.sum((centroids - vec) ** 2, axis=-1) # n_subspace, cent_cnt
    ...
    def prefill_attn(
        ...
        if CHECK_RECALL and self.layer_idx == 0:
        ...
        if self.fixbudget:
            ...
            self.topk_size = self.budget - self.sink_size - self.recent_size
        else:
            # Use ratio-based calculation (original behavior)
            total_cache = int((kv_seq_len - self.sink_size) * self.compress_ratio)
            self.recent_size = int(total_cache * self.recent_ratio)
            self.topk_size = int((kv_seq_len-self.sink_size) * self.compress_ratio * (1 - self.recent_ratio))
        # print(f"Layer {self.layer_idx}, prefill_length: {self.prefill_length}, budget: {self.topk_size}, recent_size: {self.recent_size}, topk_size: {self.topk_size}")
        self.prefill_length = kv_seq_len
        ...
        if _record_internal_stage_time():
            stage_begin('prefill_index_build', self.layer_idx)
        self.ip2l2_phi = self.build_index_cpu_multi_core_sklr(xb, centroid_cnt)
        if _record_internal_stage_time():
            stage_end('prefill_index_build', self.layer_idx)

        if _record_internal_stage_time():
            stage_begin('prefill_attn', self.layer_idx)
        ...
        if _record_internal_stage_time():
            stage_end('prefill_attn', self.layer_idx)
    ...
    def decoding_attn_GQA_euc(
        ...
        if _record_internal_stage_time():
            stage_begin('decode_retrieve', self.layer_idx)
        
        if self.layer_idx == 0:
            if _record_internal_stage_time():
                self.layer_0_start.record()
            
            self.gpu_centroids = torch.Tensor(self.centroids).to(self.device, non_blocking=True).to(torch.bfloat16)
        ...
        if _record_internal_stage_time():
            stage_end('decode_retrieve', self.layer_idx)

        if CHECK_RECALL:
            cpu_len = self.past_token_cnt - self.sink_size - self.recent_size

            k_, v_ = cache_managers[self.rank].fetch_all_key_value(
                self.layer_idx,
                cpu_len,
            )

            # fetch_all_key_value returns:
            #   [bsz, kv_head, sink + cpu + recent, dim]
            assert k_.shape[:2] == k.shape[:2], f"{k_.shape}, {k.shape}"
            assert k_.shape[-1] == k.shape[-1], f"{k_.shape}, {k.shape}"
            assert k_.shape[-2] == self.past_token_cnt, f"{k_.shape}, {self.past_token_cnt}"

            k_ = torch.cat([k_, k], dim=2)

            # topk_indices index into cpu/offloaded region.
            # In full k_ layout: sink -> cpu -> recent -> new
            # so cpu region starts from self.sink_size.
            # recall_indices = topk_indices + self.sink_size
            topk_recall_indices = topk_indices + self.sink_size
            ...
            # recent indices: [past_token_cnt - recent_size, ..., past_token_cnt - 1]
            recent_indices = torch.arange(
                self.past_token_cnt - self.recent_size,
                self.past_token_cnt,
                device=topk_indices.device,
                dtype=topk_indices.dtype,
            ).view(1, 1, 1, -1).expand(
            ...
            # current new token index after k_ = torch.cat([k_, k], dim=2)
            # k_ length is past_token_cnt + 1, so current token index is past_token_cnt
        ...
        if _record_internal_stage_time():
            stage_begin('decode_load', self.layer_idx)
        ...
        if _record_internal_stage_time():
            stage_end('decode_load', self.layer_idx)
        # if self.layer_idx == 0:
        #     pass
        ...
        # # fetch_all_key_value returns:
        # #   k_, v_: [bsz, kv_head, sink + cpu + recent, dim]
        # # order:
        # #   sink -> cpu/offloaded -> recent
        # assert k_.shape[-2] == self.past_token_cnt, f"{k_.shape}, {self.past_token_cnt}"

        # # ensure dtype matches query / current k/v
        # k_ = k_.to(device=query.device, dtype=query.dtype)
        # v_ = v_.to(device=query.device, dtype=query.dtype)

        # k = k.to(device=query.device, dtype=query.dtype)
        # v = v.to(device=query.device, dtype=query.dtype)

        # # concat current step's k/v
        # k_ = torch.cat([k_, k], dim=2)
        # v_ = torch.cat([v_, v], dim=2)

        # attn_output = flash_attn_func(
        # query.transpose(1, 2),
        # repeat(k_, num_key_value_groups, 1).transpose(1, 2),
        # repeat(v_, num_key_value_groups, 1).transpose(1, 2),
        # causal=True,
        # ).transpose(1, 2)

        if _record_internal_stage_time():
            stage_begin('decode_attn', self.layer_idx)
        # attn_output = flash_attn_func(
        #     query.transpose(1,2),
        ...
        attn_output = flash_attn_func(
            query.transpose(1,2),
            final_k_gpu.transpose(1,2),
            final_v_gpu.transpose(1,2),
            causal=True
        ).transpose(1,2)
        if _record_internal_stage_time():
            stage_end('decode_attn', self.layer_idx)
