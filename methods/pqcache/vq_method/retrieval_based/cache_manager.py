...
class GPUCacheManager:
    ...
    def init(self, key:torch.Tensor, value:torch.Tensor, layer_idx: int, topk_size: int):
        ...
        if layer_idx == 0: # We only refresh key/value cache in the first layer
            ...
            if self.fixbudget:
                self.budget = min(self.budget, self.prefill_len)
                ...
                self.local_size = self.recent_size
            ...
            # Clear GPU token-level mapping table to avoid reading random garbage on first step
            self.token_pos_record_gpu.fill_(-1)
        ...
        with torch.cuda.stream(self.D2HStream):
            ...
            if SYNC_TEST_TIME and global_timer.can_record():
                stage_begin('prefill_unload', layer_idx)
            ...
            if SYNC_TEST_TIME and global_timer.can_record():
                stage_end('prefill_unload', layer_idx)

    def add_new_token(self, new_key, new_value, layer_idx):
        layer_idx = layer_idx % self.layer_cnt
        assert new_key.shape == (self.bsz, self.n_kv_head, 1, self.dim), new_key.shape

        self.to_evict_key = self.key_buffer[layer_idx,:,:,self.local_to_evict_idx,:].clone()
        self.to_evict_value = self.value_buffer[layer_idx,:,:,self.local_to_evict_idx,:].clone()
        ...
        if SYNC_TEST_TIME and global_timer.can_record():
            stage_begin('decode_unload', layer_idx)
        ...
        if SYNC_TEST_TIME and global_timer.can_record():
            stage_end('decode_unload', layer_idx)
        
        if layer_idx == self.layer_cnt - 1:
    ...
    #     # Keep GPU cache and CPU pinned memory consistent
    #     if self.global_cache_size > 0:
    #         block_idx = self.offloaded_cnt // self.cache_block_size
    #         # Check if the offloaded token's block happens to be in the GPU LFU cache
    #         gpu_cache_pos = self.block_pos_record[layer_idx, 0, block_idx].item()
    #         if gpu_cache_pos >= 0:
    #             offset = self.offloaded_cnt % self.cache_block_size
    #             gpu_token_idx = gpu_cache_pos * self.cache_block_size + offset
    #             # Sync the new token into GPU cache to avoid reading stale data on next hit
    #             self.global_key_cache[layer_idx, :, gpu_token_idx, :, :].copy_(self.to_evict_key, non_blocking=True)
    #             self.global_value_cache[layer_idx, :, gpu_token_idx, :, :].copy_(self.to_evict_value, non_blocking=True)
    #     # end of added block
    ...
    def fetch_all_key_value(self, layer_idx, seq_len=None):
        layer_idx %= self.layer_cnt
        self.offload_events[layer_idx].wait()

        cpu_len = self.offloaded_cnt if seq_len is None else seq_len

        cpu_k = self.cpu_key_buffers[layer_idx][:, :cpu_len].cuda(non_blocking=True).transpose(1, 2)
        cpu_v = self.cpu_value_buffer[layer_idx][:, :cpu_len].cuda(non_blocking=True).transpose(1, 2)

        buf_k = self.key_buffer[layer_idx]
        buf_v = self.value_buffer[layer_idx]

        sink_k = buf_k[:, :, self.local_size:self.local_size + self.sink_size]
        sink_v = buf_v[:, :, self.local_size:self.local_size + self.sink_size]

        local_k = buf_k[:, :, :self.local_size]
        local_v = buf_v[:, :, :self.local_size]

        idx = self.local_to_evict_idx
        if idx != 0:
            local_k = torch.cat([local_k[:, :, idx:], local_k[:, :, :idx]], dim=2)
            local_v = torch.cat([local_v[:, :, idx:], local_v[:, :, :idx]], dim=2)

        k = torch.cat([sink_k, cpu_k, local_k], dim=2)
        v = torch.cat([sink_v, cpu_v, local_v], dim=2)
    ...
    def fetch_and_concat_kv_w_cache(self, indices:torch.Tensor, layer_idx):
        ...
        if self.global_cache_size > 0:
            ...
            # last_valid_block_idx = (self.offloaded_cnt - 1) // self.cache_block_size if self.offloaded_cnt > 0 else -1
            last_valid_block_idx = self.offloaded_cnt // self.cache_block_size - 1
            ...
            for i, block_idx in enumerate(q_b_idx_):
                if block2token_times[i] > 0 and block_idx <= last_valid_block_idx:
                    selected_block_indices.append(block_idx)
                    selected_old_cache_buf_pos.append(old_cache_buf_pos[i])
            ...
            with torch.cuda.stream(self.H2DStream):
                ...
                for i in range(len(selected_block_indices)):
                    ...
                    old_gpu_pos = selected_old_cache_buf_pos[i]
