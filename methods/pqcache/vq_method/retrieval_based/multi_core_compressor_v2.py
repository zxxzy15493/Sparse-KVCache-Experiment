...
def _should_patch_sklearnex():
    use_sklearnex = os.environ.get("VQ_USE_SKLEARNEX", "auto").strip().lower()
    if use_sklearnex in ("1", "true", "yes", "on"):
        return True
    if use_sklearnex in ("0", "false", "no", "off"):
        return False
    return _cpu_vendor_id() == "GenuineIntel"
...
def compute_kmeans_worker(
    ...
    #     cpu_use = int(n_core)
    #     if idx == -1:
    #         # Guard against tiny cpu_num causing empty slice
    #         safe_core = [ (cpu_num - 2) % cpu_num ] if cpu_num > 1 else [0]
    #         os.sched_setaffinity(cur_pid, safe_core)
    #     else:
    #         start_idx = core_offset + cpu_use * (idx+1)
    #         end_idx = core_offset + cpu_use * (idx+2)
    #         # Generate contiguous indices mod total core count; wraps around when cores are scarce
    #         cores_to_use = [i % cpu_num for i in range(start_idx, end_idx)]
    #         # Dedupe with set, then bind
    #         os.sched_setaffinity(cur_pid, list(set(cores_to_use)))
    # else:
    #     assert int(n_core * task_cnt) == 1, f"{n_core},{task_cnt}"
    #     cpu_use = 1
    #     # Same: use modulo to avoid out-of-range
    #     core_idx = math.floor(n_core*idx) % cpu_num
    #     os.sched_setaffinity(cur_pid, [core_idx])
        
    # Patch sklearn only on Intel by default. On AMD, sklearnex/daal4py can
    # fight the worker-level CPU affinity and stall long-context KMeans.
    if _should_patch_sklearnex():
        try:
            import sklearnex
    ...
    while True:
        ...
        for shm_set_idx in range(layer_cnt):
            ...
            for task_idx in range(task_cnt):
                # DEBUG_DISABLED: uncomment this block when measuring tensor-to-numpy latency.
                ...
                if metric == "ip":
                    ...
                    xb_array = xb_array.numpy()
                # DEBUG_DISABLED: uncomment this line with numpy_start above.
                ...
                # DEBUG_DISABLED: uncomment this block when measuring KMeans.fit latency.
                # fit_start = time.perf_counter()
                # print(
                #     f"[KMEANS_FIT_BEGIN] pid={cur_pid} worker={idx} layer={shm_set_idx} "
                #     f"task={task_idx} shape={xb_array.shape} cent_cnt={cent_cnt} "
                #     f"max_iter={max_iter} numpy_s={numpy_elapsed:.6f} "
                ...
                if threadpool_limits is None:
                    ...
                    with threadpool_limits(limits=cpu_use):
                        result = km.fit(xb_array)
                # DEBUG_DISABLED: uncomment this block with fit_start above.
                # fit_elapsed = time.perf_counter() - fit_start
                # print(
                #     f"[KMEANS_FIT_END] pid={cur_pid} worker={idx} layer={shm_set_idx} "
                #     f"task={task_idx} shape={xb_array.shape} fit_s={fit_elapsed:.6f} "
                #     f"n_iter={getattr(result, 'n_iter_', None)} inertia={getattr(result, 'inertia_', None)}",
                #     flush=True,
                # )
...
prefill_coef = {
    ...
    "qwen": [4.75329e-011, 2.53902e-06, 4.22124e-04],
...
class MultiCoreCompressor_v2:
    def __init__(
        ...
        if "llama" in model_name or "Llama" in model_name:
            ...
            self.prefill_coef = prefill_coef["qwen"]
        elif "glm" in model_name or "GLM" in model_name:
            self.prefill_coef = prefill_coef["llama"]
        else:
    ...
    def compress(self, x: torch.Tensor, cent_cnt: int, max_iter: int, layer_idx: int):
        ...
        assert x.dtype in (torch.float16, torch.bfloat16), f"Expected float16 or bfloat16, got {x.dtype}"
        ...
        if max_iter is None or max_iter == 0:
            ...
            max_iter = min(max_iter, 12)
            if layer_idx == 0:
                logger.info(f"multi-core compressor, max_iter:{max_iter}, base:{kmeans_base}, per_iter:{kmeans_per_round}, gpu_perlayer: {gpu_compute_time}") 
        else:
            if layer_idx == 0:
        ...
        for shm_set in self.shm_pool.shared_mem_block_sets:
            ...
            if not shm_set.km_task_event.is_set():
                ...
                while not shm_set.km_task_event.wait(timeout=2):
                    timeout_count += 1
                    logger.info(
                        "Waiting for km task to complete. timeout_count=%d",
                        timeout_count,
                    )

                    if timeout_count >= 20:
                        logger.error(
                            "km task did not complete after %d timeouts, killing process.",
                            timeout_count,
                        )
