...
class KVCacheH2O(object):
    def __init__(self, compress_ratio, h2o_ratio, recent_ratio, drop_ratio, sink_size = 16, show_hit_rate = True):
        print("Initing KV Cache H2O compressor")
        self.compress_ratio = compress_ratio
        self.h2o_ratio = h2o_ratio
        self.recent_ratio = recent_ratio
        self.sink_size = sink_size
        # VQ: drop a ratio of low-attn-score tokens.
    ...
    def apply(
        ...
        assert attention_score.shape == (bsz, kv_heads, kv_seq_len) # [bsz, kv_head_num, seq_len]
    ...
    def restore(
        self,
        attn_weights: torch.Tensor, # already divided by temperature on call
        num_key_value_groups: int,
    ):
        # Eviction...
        return nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32)

# Modified from H2O official code
class KVCacheH2OOfficial:
    def __init__(self, compress_ratio, h2o_ratio, recent_ratio, sink_size = 16):
        # TODO: use memory capacity instead of compression ratio?
    ...
    def apply(
        ...
        current_scores_sum = attention_score.sum(0) # (heads, k-tokens)
        ...
        if attn_tokens_all > self.cache_budget:
            ...
            if not self.recent_budget == 0:
                ...
                selected_set = self.previous_scores[:, self.sink_size:-self.recent_budget] # recent tokens are not evicted
            else:
        ...
        self.attention_masks_next = attn_mask.unsqueeze(0).unsqueeze(2) # [1, heads, 1, k_tokens + 1]
    ...
    def restore(
        ...
        current_scores_sum = attn_weights.unflatten(1, (n_kv_heads, num_key_value_groups)) \
                                        .sum(0).sum(1).sum(1) # (heads, k-tokens)
        ...
        self.attention_masks_next = attn_mask.unsqueeze(0).unsqueeze(2) # [1, heads, 1, k-tokens + 1]
...
class fullKVLimitBasedCompressor(object):
    def __init__(self, compress_ratio, h2o_ratio, recent_ratio, gqa, sink_size = 16, show_hit_rate=True, fixbudget = True, budget = 1024, recent_size = 32):
        # TODO: use memory capacity instead of compression ratio?
        self.compress_ratio = compress_ratio
        self.h2o_ratio = h2o_ratio
        self.recent_ratio = recent_ratio
    ...
    # Observe whether tokens selected by the prefill H2O strategy remain
    # important during decoding; higher hit rate => prefill picks well.
