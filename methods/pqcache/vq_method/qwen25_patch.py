...
from .retrieval_based.global_timer import (
...
__all__ = ["VQQwen2ForCausalLM", "VQQwen2Attention"]
...
def Qwen2AttentionPatch(attn: Qwen2Attention, config, idx):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # TODO: may be buggy, leave it for now
        if not first_time:
            kv_seq_len = position_ids[0][0] + 1
        cos, sin = self.rotary_emb(value_states, position_ids)
        # Qwen2's apply_rotary_pos_emb requires position_ids as the 5th argument
        
             
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids)
        pre_stage_name = 'prefill_preAttn_ffn' if first_time else 'decode_preAttn_ffn'
        ...
        if self.compressor == "original":
            ...
            if _SYNC_TEST_TIME:
                stage_end(pre_stage_name, self.layer_idx)
            ...
            stage_name = 'prefill_attn' if first_time else 'decode_attn'
            if _SYNC_TEST_TIME:
                stage_begin(stage_name, self.layer_idx)
            ...
            if _SYNC_TEST_TIME:
                stage_end(stage_name, self.layer_idx)
            ...
            if first_time:
                ...
                # print(f"prefill time {end - begin }, layer id {self.layer_idx}",flush=True)
            else:
                # begin = time.perf_counter()
                key_states = repeat_kv(key_states, self.num_key_value_groups) 
                value_states = repeat_kv(value_states, self.num_key_value_groups)
                ...
                # end = time.perf_counter()
                # print(f"decode time {end - begin }, layer id {self.layer_idx}",flush=True)
        else:
            ...
            if self.use_flash_attn and first_time:
                ...
                # vs. the true attention topk. `key_states` is GQA-repeated;
                # unrepeat to per-kv-head resolution (calc_recall re-repeats
        ...
        post_stage_name = 'prefill_postAttn_ffn' if first_time else 'decode_postAttn_ffn'
        attn_output = attn_output.transpose(1, 2).contiguous()
        if _SYNC_TEST_TIME:
            stage_begin(post_stage_name, self.layer_idx)
    ...
    if config.compressor == "h2o":
    ...
    elif config.compressor == "no_drop_lb_32":
        print("haha")
        assert attn.score_func == "sum", "full KV limited based compressor only accept sum function"
    ...
    elif config.compressor == "no_drop_lb_topp":
        print("papa")
        attn.kvcache_quantizer = fullKVLimitBasedCompressorTOPP(
            config.fixthreshold,
            config.gqa,
        )
    elif config.compressor == "no_drop_lb_topp32":
        ...
        attn.kvcache_quantizer = PqBasedSearchCompressor(
            ...
            dim = head_dim,  # Explicitly use head_dim to avoid any potential calculation errors
...
def Qwen2DecoderLayerPatch(layer: Qwen2DecoderLayer, config, layer_idx):
    def forward(self,
                hidden_states: torch.Tensor,
                attention_mask: Optional[torch.Tensor] = None,
                position_ids: Optional[torch.LongTensor] = None,
        ...
        stage_name = 'prefill_preAttn_ffn' if first_time else 'decode_preAttn_ffn'
        if _SYNC_TEST_TIME:
        ...
        stage_name = 'prefill_postAttn_ffn' if first_time else 'decode_postAttn_ffn'
    ...
    Qwen2AttentionPatch(layer.self_attn, config, layer_idx)
    return layer


def PPQwen2ModelPatch(model: Qwen2Model, config):
    def forward(
        self,
    ...
    for i in range(config.num_hidden_layers):
        model.layers[i] = Qwen2DecoderLayerPatch(model.layers[i].to(layer2device(i, config.num_hidden_layers)), config, i)
...
class VQQwen2ForCausalLM(Qwen2ForCausalLM):
    def __init__(self, config):
        a = time.perf_counter()
        super().__init__(config)
        b = time.perf_counter()

        # self.model = PPQwen2ModelPatch(self.model, config)
        ...
        # c = time.perf_counter()
        # print(f"Init model from qwen2.5 patch, Time elapsed:{c - b}, {b - a}")
