...
from vq_method.retrieval_based.global_timer import (
    ...
    reset_timer,
    get_all_stages_time,
    print_all_stages_time,
)
from breakdown_test.llama_native_timed import (
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Breakdown latency test for NATIVE (uncompressed) Llama"
    )
    ...
    parser.add_argument("--csv", type=str, default="./breakdown_test/log/native_breakdown_results.csv")
    parser.add_argument("--device", type=str, default="cuda:0")
    parser.add_argument("--attn-implementation", type=str, default="sdpa",
                        choices=["sdpa", "flash_attention_2", "eager"])
    parser.add_argument("--model-type", type=str, default="auto",
                        choices=["auto", "llama", "qwen2"],
                        help="Model family (auto: detect from name)")
...
def append_csv(csv_path, row):
    ...
    fieldnames = [
        "time", "model", "input_len", "max_new_tokens",
...
def timed_generate(model, tokenizer, input_ids, max_new_tokens):
    ...
    total = end - begin
    generated_count = int(new_tokens.numel())
    ttft = None
    tpot = None
    ...
    return total, ttft, tpot, generated_text
...
def main():
    ...
    print(f"Model type (auto-detected): {args.model_type}", flush=True)
    print(f"Attn implementation   : {args.attn_implementation}", flush=True)
    ...
    # Initialize global timer stages (same as PQCache)
    ...
    for round_idx in range(total_rounds):
        ...
        for seqlen in args.input_lens:
            ...
            total, ttft, tpot, generated_text = timed_generate(
                model=model,
                tokenizer=tokenizer,
                input_ids=input_ids,
            ...
            for stage_name in PQCACHE_ONLY_ZERO_STAGES:
    ...
    del model
    torch.cuda.empty_cache()
    logger.info("Native Llama breakdown test done.")
