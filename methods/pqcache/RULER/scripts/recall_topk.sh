#!/bin/bash
# Variant of recall.sh that exercises the **topk** (no_drop_lb) compressor with
# sink=0, recent=0 — oracle topk over the full KV cache (budget =
# high_score_budget, no sink/recent slices).
#
...
# bash recall_topk.sh MODEL_NAME BENCHMARK_NAME SYNTHETIC_TASKS PQ_BUDGET
# Example:
#   bash recall_topk.sh qwen-2.5-7b-1m synthetic niah_single_1 1024
#   bash recall_topk.sh qwen-2.5-7b-1m synthetic niah_single_1,niah_single_2 2048
#   bash recall_topk.sh qwen-2.5-7b-1m synthetic all 4096

if [ $# -ne 4 ]; then
    echo "Usage: $0 <model_name> <benchmark_name> <synthetic_tasks|all> <pq_budget>"
    echo "Example: $0 qwen-2.5-7b-1m synthetic niah_single_1 1024"
    echo "Example: $0 qwen-2.5-7b-1m synthetic niah_single_1,niah_single_2 2048"
    echo "Example: $0 qwen-2.5-7b-1m synthetic all 4096"
    exit 1
fi
...
ROOT_DIR=${ROOT_DIR:-"recall_test_topk"} # the path that stores generated task samples and model predictions.
MODEL_DIR="../.." # the path that contains individual model folders from HUggingface.
ENGINE_DIR="." # the path that contains individual engine folders from TensorRT-LLM.
BATCH_SIZE=1  # increase to improve GPU utilization

# PQ Compression Parameters (used when MODEL_FRAMEWORK='pq')
# For topk/no_drop_lb with sink=0, recent=0:
#   - The topk budget selects from the FULL kv_seq_len (no sink/recent slices).
#   - PQ_* knobs below are kept for parser compatibility (call_api.py still
#     passes them into the config), but the no_drop_lb path ignores them.
PQ_FIXBUDGET="true"   # Enable fixed budget mode
PQ_BUDGET=${4}         # Fixed budget size, read from command-line argument
PQ_COMPRESS_RATIO=0.1  # KV cache compression ratio (unused by no_drop_lb)
PQ_IMPORTANT_RATIO=0.5  # Ratio of important tokens to retrieve (unused by no_drop_lb)
PQ_RECENT_RATIO=0.5   # Ratio of recent tokens to preserve (unused by no_drop_lb)
PQ_RECENT_SIZE=0      # Number of recent tokens to keep — TOPK SETUP
PQ_SINK_SIZE=0        # Number of most recent tokens to always keep — TOPK SETUP
PQ_COMPRESSOR=${PQ_COMPRESSOR:-"no_drop_lb"}  # Compression method — TOPK SETUP
PQ_N_SUBVEC=4         # Number of PQ subvectors per head (unused by no_drop_lb)
PQ_N_SUBBITS=8        # Bits per PQ subvector (unused by no_drop_lb)
PQ_TOPR=1             # Top-k tokens to retrieve during decoding (unused by no_drop_lb)
...
PQ_FIXTHRESHOLD=0.85 # Fixed threshold for topp attention (unused by no_drop_lb)

# System parameters
DEVICE=0              # GPU device ID
MAX_CPU_IN_USE=32     # Number of CPU cores for K-means (unused by no_drop_lb)
...
# For topk/no_drop_lb the `pq` framework in config_models.sh uses PQModel which
# loads the model locally — no vllm/trtllm/sglang server is started, so the
# branches below are no-ops for the models that resolve to MODEL_FRAMEWORK=pq.
...
for MAX_SEQ_LENGTH in "${SEQ_LENGTHS[@]}"; do
    ...
    # Build model name with PQ suffix. no_drop_lb is not a topp compressor, so
    # the budget-based suffix path applies.
    PQ_MODEL_NAME=""
    ...
    DATA_DIR="../../../../benchmarks/Ruler_recall/${MODEL_NAME}/${BENCHMARK}/${MAX_SEQ_LENGTH}/data"
