...
def limit_cuda_memory(fraction=0.3, device=0):
    if torch.cuda.is_available():
        torch.cuda.set_per_process_memory_fraction(fraction, device=device)
        logger.info(f"Set CUDA memory limit to {fraction:.0%} on device cuda:{device}")
