...
def reset_sample_cache_state(model):
    ...
    if "llama" in model_type:
        import evict.h2o.experiment.h2o_kv.modify_llama as modify_llama
    elif "glm" in model_type:
        import evict.h2o.experiment.h2o_kv.modify_glm as modify_glm

    for attn in iter_attention_modules(model):
        if hasattr(attn, "kv_cache") and attn.kv_cache is not None and hasattr(attn.kv_cache, "hh_score"):
            attn.kv_cache.hh_score = None
        if hasattr(attn, "kv_cache_evictor") and attn.kv_cache_evictor is not None and hasattr(attn.kv_cache_evictor, "hh_score"):
            attn.kv_cache_evictor.hh_score = None
...
@torch.no_grad()
def streaming_inference_heavy_hitter(model,model_name_or_path, tokenizer,dataset,dataset_name, samples,output_path,prompt_format,max_gen_len=1000):
    device = model.device
    done_count = 0
...
def main(args):
    ...
    if args.enable_h2o_cache:
        from evict.h2o.experiment.h2o_kv.enable_h2o import enable_h2o
        enable_h2o(model, args)
        print(f"H2OKVCache-LayerWise: {args.heavy_hitter_size}, {args.recent_size}")
