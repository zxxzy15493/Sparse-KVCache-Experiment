...
def _acc_event(kv, start_ev, end_ev):
    end_ev.synchronize()
    ms = start_ev._orig_elapsed(end_ev)
    return ms
...
def common_timing_and_evict(self, query_states, key_states, value_states, past_key_value, bsz, q_len, sin, cos):
    ...
    if q_len > 1:
        ...
        ms = _acc_event(self.kv_cache, self.kv_cache.prefill_writeback_start_event, self.kv_cache.prefill_writeback_end_event)
        self.kv_cache.prefill_writeback_event_time += ms / 1000.0
        ...
        for i in range(0, q_len, chunk_size):
            ...
            causal_mask = row_indices >= (col_indices - (kv_seq_len - q_len))
            attn_chunk.masked_fill_(~causal_mask.unsqueeze(0).unsqueeze(0), float("-inf"))
            attn_chunk = F.softmax(attn_chunk, dim=-1, dtype=torch.bfloat16)
            current_scores += attn_chunk.sum(dim=-2)

        self.kv_cache.hh_score = current_scores

        if kv_seq_len <= self.kv_cache.cache_size:
            k_comp, v_comp = key_states, value_states
        else:
            history_len = kv_seq_len - self.kv_cache.recent_size
            history_scores = self.kv_cache.hh_score[:, :, :history_len]
            _, keep_topk_idx = torch.topk(history_scores, self.kv_cache.h2o_size, dim=-1)
            ...
            self.kv_cache.hh_score = torch.gather(self.kv_cache.hh_score, -1, keep_idx)
        ...
        ms = _acc_event(self.kv_cache, self.kv_cache.decode_writeback_start_event, self.kv_cache.decode_writeback_end_event)
        self.kv_cache.decode_writeback_event_time += ms / 1000.0
        ...
        attn_weights = F.softmax(attn_weights, dim=-1, dtype=torch.bfloat16)
        current_scores += attn_weights.sum(dim=-2)
        self.kv_cache.hh_score = torch.cat([self.kv_cache.hh_score, current_scores[:, :, -1:]], dim=-1)
        ...
        ms = _acc_event(self.kv_cache, self.kv_cache.retrieve_start_event, self.kv_cache.retrieve_end_event)
        self.kv_cache.retrieve_event_time += ms / 1000.0
        ...
        ms = _acc_event(self.kv_cache, self.kv_cache.decode_attn_start_event, self.kv_cache.decode_attn_end_event)
        self.kv_cache.decode_attn_event_time += ms / 1000.0
...
def h2o_forward_timing_patch(self, hidden_states, attention_mask=None, position_ids=None, past_key_value=None, **kwargs):
...
def enable_h2o_time(model, heavy_hitter_size=992, recent_size=32):
    global TOTAL_LAYERS
    try:
    ...
    total_budget = heavy_hitter_size + recent_size
    num_layers = 0
    ...
    for name, module in model.named_modules():
        if (LlamaDecoderLayer and isinstance(module, LlamaDecoderLayer)) or (Qwen2DecoderLayer and isinstance(module, Qwen2DecoderLayer)):
            ...
            kv_cache = H2OKVCache(recent_size=recent_size, h2o_size=heavy_hitter_size, k_seq_dim=2, v_seq_dim=2)
            ...
            attn_module.forward = types.MethodType(h2o_forward_timing_patch, attn_module)
    ...
    print(f"H2O Co-Timing Profiler Enabled. Patched {patched} Layers (Budget={total_budget}, H2O={heavy_hitter_size}, Recent={recent_size}).")
