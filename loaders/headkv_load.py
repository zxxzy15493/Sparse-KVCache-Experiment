...
def load_model_and_tokenizer(args):
    """Load a model and attach HeadKV adaptive head-level KV compression."""
    headkv_root = Path(__file__).resolve().parents[1] / "methods" / "HeadKV"
    if str(headkv_root) not in sys.path:
        sys.path.insert(0, str(headkv_root))
    ...
    for key, value in method_defaults("headkv").items():
        if key != "budget":
    ...
    replace_llama("ReasonKV")
    replace_qwen2("ReasonKV")
    ...
    if "glm" in model_name.lower():
        replace_chatglm("ReasonKV", model)
    ...
    for i in range(n_layers):
        ...
        attn.config.head_choice = method_args.head_choice
        attn.config.beta = method_args.beta
        attn.config.temp = method_args.temp
